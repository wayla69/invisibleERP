...
> **Status:** Phases 0–1 DELIVERED (2026-07-12) — the service-size accretion ratchet is live in the
> `build` gate, and the BI report dispatch is a provider registry: 52 report branches moved to
> module-owned `*-bi-reports.ts` generators (`bi-generate.service.ts` 777→336 LOC and OFF the
> grandfathered list). Phases 2–5 below remain proposals. This is the follow-up to docs/38 (god-service
