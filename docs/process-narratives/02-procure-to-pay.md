...
9. **Vendor statement of account.** A per-vendor **statement** (`GET /api/finance/ap/statement?vendor=&from=&to=`) lists an **opening balance** struck before the window, every bill (charge) and approved disbursement (payment) in date order with a **running balance**, and a **closing balance** — used to reconcile to the supplier's own statement before paying.
