...
|---|---|---|---|
| 0.16 | 2026-07-07 | Platform / Tax | **หนังสือรับรองการหักภาษี ณ ที่จ่าย (50-ทวิ) PDF layout matched to the RD's official ล.ย.02 template (presentation-only).** `whtCertificateHtml` (`tax-docs-pdf.service.ts`) is restructured to mirror the printed form: the payer/payee blocks render as bordered side-by-side columns each with a **13-cell boxed Tax-ID grid** (`taxIdBoxes`); the ภ.ง.ด. selector shows all **7** numbered checkboxes (1)–(7) in the official order (1ก / 1ก พิเศษ / 2 / 3 / 2ก / 3ก / 53) instead of the previous 4; the income table's row 4 splits into **(ก) ดอกเบี้ย ม.40(4)(ก)** and **(ข) เงินปันผล ม.40(4)(ข)** as two lines (was one combined row) with the standard dividend tax-credit-rate sub-note (30%/25%/20%/other — presentational only, no new data captured); table column headers and row-5/6 wording now match the printed form; the ผู้จ่ายเงิน condition checkboxes gain their (1)–(4) numbering; signature block and a Tax-ID footnote added. No schema, endpoint, computation, or GL change — `total_paid`/`total_wht`/`pnd_type`/`wht_condition` are unchanged and still drive the same fields. **No migration, no new/changed RCM control** (reinforces the existing tax-document-integrity control, TAX-01/TAX-03). ToE: `taxdocs` **93** unchanged/green (string-level assertions on "มาตรา 50 ทวิ" / บาทตัวอักษร still hold). |
