...
| Related RCM controls | RET-01, RET-02, RET-03, REV-06, GL-01, TAX-07; SoD R08, R12 |
...
9. **GL reversal (idempotent, source RTN).** A balanced reversing journal posts **Dr 4000 Revenue + Dr 2100 VAT Output / Cr 1000 Cash** (or **Cr 2200 Customer Deposits** for store credit). Re-posting is idempotent on source RTN (**GL-01**, **RET-02**). **Credit note (ใบลดหนี้ ม.86/10) auto-issued on the return (#2).** When the returned sale carries an **Issued tax invoice**, the return also issues the matching **ใบลดหนี้** (prefix **CN-**, status **Issued**, `source_ref = RTN-`) so the **output-VAT (ภ.พ.30) report is reduced** for the return — it references the original invoice (`original_doc_no`), carries the returned net/VAT/total, and **LINKS (does not re-post) the RTN journal** (`gl_entry_no`): the cash reversal above **is** the GL effect, so the credit note posts **no additional GL**. This is distinct from the standalone maker-checker **AR** credit note (which posts Dr revenue+VAT / Cr 1100 AR — wrong for a cash POS sale, see **TAX-07**); here the return is the authorising event and the note is Issued immediately (a large refund is separately gated by REV-16). Idempotent per return; **skipped** (`credit_note_no` stays null) when the sale has no Issued invoice — e.g. a Comp/unbilled sale. Mechanics: `06-tax-compliance.md`.
...
|---|---|---|---|
| 0.4 | 2026-07-18 | Platform | **Credit note (ใบลดหนี้ ม.86/10) auto-issued on a POS return (#2 — strengthens RET-02/TAX-07 completeness; no new numbered control, no migration).** §7 step 9. A return against a sale that carries an **Issued tax invoice** now auto-issues the matching **ใบลดหนี้** (prefix **CN-**, status Issued, `source_ref = RTN-`, `original_doc_no` = the sale invoice) so the return is reflected in the **output-VAT (ภ.พ.30)** report — closing the prior gap where the RTN journal reversed VAT in the GL but the VAT report still counted the full original supply. Critically it posts **no additional GL**: the note **LINKS** the RTN journal (`gl_entry_no`) rather than re-posting, since the cash reversal already **is** the GL effect (contrast the standalone maker-checker AR credit note under TAX-07, which posts to 1100 AR — wrong for a cash sale). Issued immediately (the return is the authorising event; large refunds still gated by REV-16); idempotent per return; skipped when the sale has no Issued invoice. New method `TaxInvoiceService.issueCreditNoteForReturn` (runs on the return's tx → atomic); `ReturnsService` wires it and populates `pos_returns.credit_note_no`. ToE: `returns` harness 20→25 (CN issued, type/status/refs/amounts, **no** CN-source journal, idempotent, no-invoice→null, TB balanced). RCM census unaffected. Mechanics in `06-tax-compliance.md`; manual `01-sales-and-pos.md` + UAT `02-order-to-cash-uat.md` (UAT-O2C-561) + traceability matrix updated. |
| 0.3 | 2026-07-16 | Platform / Security | **Refund maker-checker threshold made cumulative (pentest P6; control REV-16, no new control/migration).** The REV-16 approval gate was evaluated **per-call** on `dto.amount`, so a large refund could be split into sub-threshold refunds (e.g. ฿9,000 as 10 × ฿900) that each executed immediately with no independent approval — the second-person control was skipped even though no money was created (the over-refund cap still held). The gate now keys on the **cumulative** refunded amount against the payment (settled refunds + already-pending requests + this amount); once the running total reaches the threshold the refund parks as a `PendingApproval` request until a different user approves. The over-refund invariant remains enforced under the `FOR UPDATE` lock. ToE: `refund-approval` harness +1 (a ฿3,000 payment refunded 600 then 600 — the first runs immediately, the second parks `PendingApproval` — 11 ✓). No RCM/count change. |
