...
| Related RCM controls | IC-01, IC-02, IC-03, IC-04, CONS-01, CONS-02, CONS-03, CONS-04, CON-03 (elimination integrity / run→post maker-checker), CON-04 (segment reporting), CON-05 (dual-rate translation / CTA-OCI / consolidated SCF), TRE-05 (intercompany-loan 1155/2155 + interest elimination — see PN-33 §7quater), FX-04, GL-01, REC-03; SoD R01, R07, R05 |
...
6. **Consolidation run (decision point, maker-checker).** `POST /api/consolidation/groups/:groupId/run` (period `YYYY-MM`) requires the **`approvals`** permission, enforcing initiate-vs-approve separation (**R07**); an unknown group → `GROUP_NOT_FOUND` (`404`); a group with no entities → `NO_ENTITIES` (`400`). The run builds `consolidation_run_lines` (**CONS-02**):
   ...
   - **Elimination lines** — for IC where both tenants are in the group, eliminate **1150** / **2150**; and (Track C Wave 4, **TRE-05**) for an **intercompany loan** whose both legs are in the group, eliminate the loan **1155** / **2155** receivable/payable pair **and** the IC interest (**4700** creditor income / **5900** debtor expense) for the period, so the group balance sheet nets the intra-group loan to zero and group finance cost/income nets out (`ic_loan_eliminations` reported on the run).
...
| 7 | Intercompany LOAN + its interest not eliminated → group grosses up an intra-group loan / double-counts finance cost | Elimination lines remove 1155/2155 + 4700/5900 for in-group IC loans (per period) | Auto / Det | **TRE-05**, CON-03 | IC-loan elimination lines (`ic_loan_eliminations`) |
...
| 0.9 | 2026-07-16 | Platform | **Schedulable consolidation staging (docs/50 Wave 2 B3; no new control/migration).** New action job `consolidation_run` (NEW `consolidation/consolidation-bi-reports.ts`, docs/46 Phase 1 provider pattern; filters `{period?, group_id?}`, default period = the just-ended business month) stages each group's run via the SAME `runConsolidation` — every gate binds unchanged (REC-03 `IC_RECON_NOT_APPROVED`, `NO_ENTITIES`, `ALREADY_POSTED` frozen) and the sweep is **fault-isolated per group** (a gated group records its outcome; the others still stage). **Auto-Draft only** — CON-03 maker-checker posting is untouched. ToE `consolidation` 42→47 (posted-period no-op; IC-gate no-op; gate cleared → staged Draft). UAT-GL-195. |
| 0.8 | 2026-07-12 | Treasury / Controller | **Intercompany-loan elimination on consolidation (Track C Wave 4, TRE-05).** `runConsolidation` now also eliminates, per period, an in-group **intercompany loan's** receivable/payable pair (**1155** / **2155**) **and** its interest (**4700** creditor income / **5900** debtor expense) — mirroring the trade-IC **1150** / **2150** elimination — so the group balance sheet nets the intra-group loan to zero and group finance cost/income nets out (CON-03 balanced-TB integrity preserved; the run reports `ic_loan_eliminations`). Source: the new `modules/treasury-pool` IC-loan register (see PN-33 §7quater, migration 0357, COA 1155/2155). No change to the IC (1150/2150) or NCI/FX logic. ToE: `treasury-pool` harness (46 checks) proves the group nets to zero; `consolidation` harness (42) stays green. |
