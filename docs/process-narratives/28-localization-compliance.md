...
| 0.8 DRAFT | 2026-07-17 | Platform | **EN coverage extension round 2 — auth + public end-customer surfaces follow the user's language (no API/DB/permission change, no new control).** The `LanguageProvider` moves to the ROOT layout (one app-wide instance; the (internal)/portal copies removed) so **every** page has locale context — for signed-out/public visitors the device's `localStorage` cache alone applies (default th). Wired + translated: **auth** (login incl. PIN/SSO/MFA, signup-request, change-password, SSO callback; new `i18n-catalog/auth.ts`, `auth.*`) with a `LanguageToggle` on login/signup; **diner QR ordering** `/qr/[token]` (its local th/en item-name toggle now drives the global locale; item statuses translate via the stable `kds_status` codes with `status_th` fallback) + `/qr/start`; **channel-order tracker** `/track/[token]` (+toggle); **NPS** `/nps/[token]`; **loyalty member app** `/m` (+toggle; its `mapi` fetch layer now also picks server `message`/`messageTh` by locale) — new `i18n-catalog/public.ts` (`pub.*`/`mb.*`, ~190 keys th+en). Kept **bilingual-by-design**: `/q` scan resolver (server component), customer pole display `/display` (both audiences see it at once; labels completed), `global-error` (renders without providers), `/legal/privacy`. Doc-sync: manual 00 §5 + FAQ updated (portal/auth/public now follow the language). UAT unchanged (translation coverage; rev 0.4 precedent). |
