...
6. **Offline-to-online sync reconciliation.** On reconnect, offline-captured sales must reach the central ledger exactly once. Each synced sale is tagged to its branch and folded into the fiscal hash-chained journal of `20-restaurant-operations.md`. There are **two offline capture paths, both idempotent on `(tenant, client_uuid)`** via the `pos_offline_sync` dedup ledger: the **portal/inventory POS** replays to `POST /api/portal/pos/offline-sync` (`item_id` lines), and the **touch register** (`/pos/register`, menu `sku` lines) replays its quick cash sales to `POST /api/restaurant/offline-sync` — which re-runs the normal restaurant order→checkout (the server re-prices + 86-checks; a re-sent batch returns `duplicate` and never double-posts the sale or its GL). A failed op is a retryable tombstone (it never blocks a later replay), so a transient error is not a lost sale. Terminal-side capture is resilient to the two common real-world failure shapes: the register **snapshots the menu on-device** (localStorage **and**, POS-6, the service worker now serves `GET /api/menu` stale-while-revalidate — caching DATA, never HTML navigations, which stay network-first) so a reload/reboot mid-outage still renders a sellable menu, and a quick sale whose order-creation call fails at the **network level while the browser still reports online** (router up, internet down — `navigator.onLine` cannot see it) falls back to the same offline queue automatically; HTTP-level rejections (validation, 86'd item, expired session) still surface to the cashier and are never queued. **POS-6 extends the same idempotent spine to DINE-IN mutations** (previously online-only): while offline the register captures the dine-in order lifecycle — **open** (table + items), **add** (items), **fire** (send to kitchen) — as ops that share a client-generated `order_uuid` and replay to `POST /api/restaurant/offline-sync/dinein`, ordered by `client_seq` so an `open` always applies before the `add`/`fire` ops that resolve their server order by that `order_uuid` (migration `0301` adds `op_type`/`order_no`/`order_uuid` to `pos_offline_sync`). **Settlement stays ONLINE**: the replay creates and fires the order (no GL until settled), and the cashier settles it through the normal checkout once reconnected — so an offline outage no longer blocks the kitchen, and a reload while offline never loses the till (the register is an installable PWA: `manifest.webmanifest` + the `sw.js` registration). *Control: BRANCH-03 — no lost or duplicated offline transactions (quick sales **or** dine-in ops); synced count and value reconcile to terminal-side capture counts (cross-ref REC-01 tie-out).*
...
6d. **Hub cash session (till / Z-report) up-sync (docs/41 Phase 2c).** The hub pushes each **closed**
   ...
   *Where the cash figure comes from (verified, not assumed):* the **tender** — not the sale header —
   ...
   (`cashDue = total + tip`). The **native** `aggregateTill` now applies the same rule (a cash tip is
   drawer cash; a tip on a card tender is not), so an online close and a hub close agree — see
   PN-20 / REV-13.
...
6f. **Hub kitchen-waste replay (docs/41 Phase 2c-2).** Waste logged on a hub posts **Dr 5810 Scrap/Waste
   Loss / Cr 1200 Inventory** on the hub's ledger. The pusher sends each `waste_log` document (item, qty,
   reason code, unit cost), signed with the same HMAC, to `POST /api/hub/ingest-waste`, which replays it
   through the **same `WasteService`** a native entry uses — so every guard still applies, including the
   **INV-07 perpetual-item guard** (a perpetual-tracked item must go through the approved write-off; such a
   document fails **loudly** and stays visible in `hub_push_log` rather than relieving inventory
   off-control). The hub's `waste_no` **is** the document identity on both ledgers: a re-push returns the
   stored row (`duplicate`) and neither decrements stock nor posts GL twice.

   *Valuation, stated not assumed:* `unit_cost` is the hub kitchen's ingredient cost. Unlike a sale — which
   the cloud re-prices from its own master — the cloud has **no independent cost basis** for a
   non-perpetual ingredient, so the hub's figure is accepted and carried into the GL.
   *Control: **BRANCH-06**.*
6g. **Hub stocktake replay (docs/41 Phase 2c-2).** Only **POSTED** count sheets are pushed — a Draft
   ...
   migration `0318`; the native post previously stored only the counter). `POST /api/hub/ingest-stocktake`
...
| 0.9 | 2026-07-10 | Platform | **Offline dine-in + installable PWA register (POS-6 — BRANCH-03 widened, no new numbered control).** §7 step 6: the offline outbox spine now covers **DINE-IN** mutations (previously online-only) — while offline the register captures **open** (table + items) / **add** (items) / **fire** (send to kitchen) as ops sharing a client `order_uuid`, replayed to the new **`POST /api/restaurant/offline-sync/dinein`** (`RestaurantOfflineSyncService.syncDineInBatch`), ordered by `client_seq` so `open` applies before the `add`/`fire` that resolve their server order by `order_uuid`; idempotent on `(tenant, client_uuid)` (a replayed op → `duplicate`, never re-fires a line). **Settlement stays ONLINE** (no GL until the cashier checks out the replayed order). Migration **`0301`** adds `op_type`/`order_no`/`order_uuid` to `pos_offline_sync` (columns only — existing RLS/grant cover them; no new table). The register is an **installable PWA** (`manifest.webmanifest` + `sw.js` registration) and the SW now caches **`GET /api/menu`** stale-while-revalidate (DATA, not HTML — HTML stays network-first per the deploy-safe rule), so a reload while offline keeps a sellable menu and does not lose the till. Client: `lib/register-offline.ts` gains a dine-in IndexedDB store + `enqueueOfflineDineInOrder`/`syncDineIn` (reuses the existing island — `use-client` baseline unchanged); the register's offline dine-in path now **queues open+fire** (ส่งครัวออฟไลน์) instead of blocking. ToE: `restaurant-offline` harness → **14 checks** (open+fire creates+fires, replay all-duplicate/no double-fire, add resolves by `order_uuid`, add-before-open → `OFFLINE_ORDER_NOT_SYNCED`, out-of-seq batch sorts, tenant-scoped resolution). UAT-O2C-343..322. |
...
| 0.9 | 2026-07-10 | Platform | **Native till aligned with the hub rule (REV-13 correctness).** `aggregateTill` now counts a **cash tip** as drawer cash (`payments.amount` excludes the tip while the sale’s GL debits 1000 for `total + tip`); a tip on a **card** tender is still excluded. Before this, every close with a cash tip read **“over” by exactly the tip** — a false variance that could trip the REV-13 material-variance maker-checker. §7 6d caveat retired: an online close and a hub close now agree. ToE: `cashreport` harness (34 — cash tip counted, card tip excluded, drawer counted at 127 closes with variance 0); regression `pos-p0` 25, `restaurant` 167, `tips` 10, `splitbill` 16. UAT-O2C-316. |
| 0.9 | 2026-07-10 | Platform | **Hub kitchen-waste replay (docs/41 Phase 2c-2 — NEW control BRANCH-06).** New §7 step 6f: `POST /api/hub/ingest-waste` (HMAC, fail-closed) replays each hub `waste_log` document through the **same `WasteService`** as a native entry (positive qty, known reason, INV-07 perpetual guard), idempotent on the hub's `waste_no` (re-push ⇒ `duplicate`, no second stock decrement, no second JE); `hub_push_log` gains `doc_type='waste'`. Before this, waste posted 5810/1200 **only on the box** — central COGS understated, inventory drifting, shrinkage invisible to HQ. Valuation basis documented (the cloud cannot re-derive a non-perpetual ingredient cost). ToE: `hub-snapshot` harness → **62 checks**; regression `waste` 9, `stock-ops` 21. UAT-O2C-319. |
| 0.11 | 2026-07-16 | Platform | **Loyalty-redemption sales now REPLAY hub→cloud with a cloud-side clamp (docs/50 Wave 4 C4 — closes docs/41 Phase 2c's `LOYALTY_REDEEM` skip; no new control/migration).** The hub pusher no longer skips a `points_used > 0` sale: it sources the member from the hub's OWN points ledger (the sale's `Redeem` row) and emits additive `member_id`/`redeem_points` op fields (ingest zod passes them through — the HMAC signs the full sales array). The CLOUD clamps to ITS balance before checkout ("adjusted at sync": `REDEEM_CLAMPED`/`MEMBER_NOT_FOUND`/`LOYALTY_UNAVAILABLE` surfaced on the sync result; the revenue-bearing sale never fails over points drift), then the native redeem lock + LYL-22 idempotency apply — a re-push never double-deducts or double-earns. A redemption with no traceable member stays a defensive skip (`LOYALTY_REDEEM_NO_MEMBER`). **Fiscal hash-chain "splice" is consciously NOT built:** the cloud already re-appends its own book-of-record chain per replayed sale (PN-20); splicing hub hashes would fork it — the hub keeps its local chain, verifiable hub-side. ToE `hub-snapshot` 73→79 (6 C4 checks). UAT-O2C-515..516 + traceability. |
| 0.10 | 2026-07-10 | Platform | **Hub stocktake replay (docs/41 Phase 2c-2 — NEW control BRANCH-07; migration 0318).** New §7 step 6g: only POSTED sheets replay, each naming **both** humans; the cloud refuses a sheet whose counter is its poster (`STOCKTAKE_NOT_SEGREGATED`), so the machine principal cannot launder a self-approved count (SoD R11 / INV-04). `postStocktake` now persists `posted_by`/`posted_at` — the segregation is *evidenced* on the document, on either ledger, instead of only being enforced in the moment. Idempotent on `(tenant, st_no)`; blocked sheets stay visible in `hub_push_log`. Depends on the 0316 tenant-isolation fix. ToE: `hub-snapshot` harness → **73 checks**; regression `stock-ops` 27. UAT-O2C-343. |
