...
6g. **Hub stocktake replay (docs/41 Phase 2c-2).** Only **POSTED** count sheets are pushed — a Draft
   ...
   migration `0310`; the native post previously stored only the counter). `POST /api/hub/ingest-stocktake`
