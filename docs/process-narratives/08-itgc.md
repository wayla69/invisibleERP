...
5. **Secrets at rest.** TOTP seeds and webhook secrets are AES-256-GCM encrypted; `APP_ENC_KEY` is required in prod (boot fails without it) (**ITGC-AC-04**). Secrets management/rotation via vault is the open remediation (**ITGC-AC-12**, gap).
...
8. **Audit trail.** Every mutating request is captured in append-only `audit_log` (actor, tenant, action, IP, requestId, status, meta); `UPDATE`/`DELETE` on `audit_log` are rejected by a DB trigger — tamper-evident (**ITGC-AC-10**). The POS fiscal journal is hash-chained (**ITGC-AC-11**). Password/session hardening (**ITGC-AC-07**) and named DB users (**ITGC-AC-13**) are partial/open remediations.

### 7.B Change management & SDLC

9. **Peer review + branch protection.** All code changes go via peer-reviewed PR; branch protection on `main`, required reviews, and required CI before merge (**ITGC-CM-01**, partial — enforce no self-merge).
...
11. **Deploy segregation (decision point).** Production deploy requires a separate approval; the **deployer ≠ author** (**ITGC-CM-03**, gap to configure).
12. **Traceability & emergency change.** Every change is traceable ticket → PR → test → deploy (**ITGC-CM-04**); emergency changes follow a runbook with retroactive approval SLA (**ITGC-CM-05**).
...
14. **Backup + restore (decision point).** Automated DB backups with a **tested quarterly restore** — recovery proven, not just scheduled (**ITGC-OP-01**, gap).
15. **DR/BCP.** DR/BCP plan with defined RTO/RPO and periodic test (**ITGC-OP-02**, gap).
16. **Monitoring & incident.** Monitoring/alerting (APM/errors) with on-call and an incident log (**ITGC-OP-03**, partial).
17. **Batch-job monitoring.** Scheduled financial jobs (`pg-boss`: recurring billing, FX revaluation, subscriptions) are monitored with failure alerting + review so postings/billing are not silently missed (**ITGC-OP-04**, gap).
...
| 5 | Secrets hardcoded/un-rotated | Vault + rotation (remediation) | Prev / Manual | ITGC-AC-12 (gap) | Vault config |
...
| 9 | Unauthorized/untested code to prod | PR review + branch protection + CI gate | Prev / Hybrid | ITGC-CM-01 | PR approvals; CI logs |
| 10 | Ad-hoc prod schema change | Migration-only DDL (Drizzle journal) | Prev / Hybrid | ITGC-CM-02 | Migration log |
| 11 | Developer self-deploys to prod | Deploy approval; deployer ≠ author | Prev / Manual | ITGC-CM-03 (gap) | Deploy approvals |
| 12 | Change not traceable | Ticket → PR → deploy linkage | Det / Manual | ITGC-CM-04 (gap) | Traceability |
| 13 | Control logic regresses unnoticed | Automated tests as control evidence; CI archive | Det / Auto | ITGC-SD-03 | CI reports |
| 14 | Data loss, no recovery | Backups + tested quarterly restore | Prev / Hybrid | ITGC-OP-01 (gap) | Restore-test record |
| 15 | Prolonged outage | DR/BCP with RTO/RPO + test | Prev / Manual | ITGC-OP-02 (gap) | DR test |
| 17 | Financial batch jobs fail silently | Batch-job monitoring + alerting | Det / Auto | ITGC-OP-04 (gap) | Job logs |
