...
**no-op**: a partial unique index **`uq_scm_retrain_run`** on `scm_plan_runs` (migration **0476**) plus the
generalized `executePlanRun` run guard short-circuit the second run, mirroring the nightly guard of §7.7.

**Migration 0476** is additive on **existing** tables — no RLS loop and no new grant. It adds a
