...
| Related RCM controls | GL-01, GL-02, GL-03, GL-04, GL-05, GL-06, GL-07, GL-08, GL-09, GL-10, GL-11, GL-15, GL-16, GL-19, GL-21, LSE-01, REC-01, REC-02, REC-03, REC-04, GOV-01, CON-01, CON-02; SoD R05, R06 |
...
| 16 | Item / COGS posting accounts are hard-coded, or a per-item override points at a bad account | Item-posting account determination (extends GL-12 to the item grain): opt-in `posting_determination` flag (default OFF ⇒ literal parity); `AccountDeterminationService` resolves item → item_category → hard-coded literal; the inventory sub-ledger routes its inventory/COGS legs through it and `reconcile()` sums the whole inventory-account set so the GL-14 tie-out holds; a resolved override that isn't a real, **postable** CoA account is rejected `INVALID_POSTING_ACCOUNT` (fail-closed) | Prev / Auto | GL-21 | `basics` GL-21 ToE (parity off; routed COGS on; reconcile ties; invalid-account reject) |
