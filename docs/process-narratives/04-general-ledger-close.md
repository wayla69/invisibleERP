...
| Related RCM controls | GL-01, GL-02, GL-03, GL-04, GL-05, GL-06, GL-07, GL-08, GL-09, LSE-01, REC-01, REC-02, REC-03, REC-04, MON-01, CON-01, CON-02; SoD R05, R06 |
...
| 7 | A maker-checker item sits unapproved for weeks (segregation defeated in practice; cash tied up, books mis-stated, no one watching the queue) | **Pending-approvals aging monitor** — one read ages every maker-checker queue (all Draft JEs + inventory write-offs + AP payments); items past the SLA are exceptions (`all_clear`, `stale_count`) | **Det / Auto** | **MON-01** | Aging monitor (`GET /api/finance/approvals/aging`); `compliance` harness |
...
| 0.4 | 2026-06-26 | Platform | **MON-01 — pending-approvals aging monitor.** Step 7: `FinanceService.pendingApprovalsAging` on `GET /api/finance/approvals/aging` ages every maker-checker queue in one read — every Draft journal entry (auto-covers GL-05/PAY-03/FA-08/FA-09/BANK-02/FX-04) + INV-07 inventory write-offs + AP-PAY vendor payments — control-tagged, attributed, valued, bucketed; items past the SLA (default 7 days, `?stale_days`) are exceptions with `all_clear`/`stale_count` + per-control roll-up. New RCM control **MON-01** (RCM now 85); control matrix gains a step-7 monitoring row. Read-only detective; no migration. ToE: `compliance` (a Draft item surfaces aged & control-tagged, not stale when fresh; tightening the SLA flags it as an exception). |
