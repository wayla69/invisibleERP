...
| 0.4 | 2026-06-26 | Platform | **MON-01 — pending-approvals aging monitor.** Step 7: `FinanceService.pendingApprovalsAging` on `GET /api/finance/approvals/aging` ages every maker-checker queue in one read — every Draft journal entry (auto-covers GL-05/PAY-03/FA-08/FA-09/BANK-02/FX-02) + INV-07 inventory write-offs + AP-PAY vendor payments — control-tagged, attributed, valued, bucketed; items past the SLA (default 7 days, `?stale_days`) are exceptions with `all_clear`/`stale_count` + per-control roll-up. New RCM control **MON-01** (RCM now 85); control matrix gains a step-7 monitoring row. Read-only detective; no migration. ToE: `compliance` (a Draft item surfaces aged & control-tagged, not stale when fresh; tightening the SLA flags it as an exception). |
