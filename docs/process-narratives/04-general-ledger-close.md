...
14. **Chart of Accounts management (WS1.1 — GL-11).** The Chart of Accounts is now **master data** — editable via API by authorised users (`gl_coa` permission, held by FinancialController and Admin) rather than hardcoded. The account hierarchy is: **account_groups** (tenant-scoped groups, `NULL tenant_id` = global template visible to all tenants) → **accounts** (the canonical posting universe, extended with Thai name, group link, control flags, normal balance, postability, dimension requirements, and effective dates). Key controls:
    ...
    - **Per-tenant presentation curation** (overlay write — `PATCH /api/ledger/accounts/:code/overlay`, permission `gl_coa`): a company's finance user renames (EN/TH), re-groups (`group_label`), re-orders (`sort_order`), or toggles `active` on the accounts of **their own tenant's chart** — writing only the `tenant_accounts` overlay (step 15), which **RLS pins to the caller's tenant**. This is **presentation-only**: postings and the `?all=true` canonical universe are unaffected, exactly like GL-10. Because the overlay curates the **existing** canonical universe, it can never mint or recode a *master* account — curating a code that is not in the canonical chart is a platform-administrator (HQ) operation and is rejected `COA_ADMIN_ONLY`. A curated-off (`active=false`) account is hidden from the default chart but stays visible under `?include_inactive=true` so it can be re-activated. Surfaced in the **ผังบัญชี** tab of `/accounting` for `gl_coa` users (rename dialog, group label, active toggle, ↑/↓ re-order); master-code create/deactivate remain the canonical routes above (Admin/HQ). Deletion of a canonical account is never offered.
...
| 14 | CoA changed without authorisation; code changed after postings; account deactivated with live balance; direct JE bypasses sub-ledger on a control account; a tenant silently invents/recodes a master account via presentation curation | CoA Change Control: `gl_coa` permission required; code-change blocked if postings exist (`CODE_HAS_POSTINGS`); deactivation blocked if non-zero balance (`ACCOUNT_HAS_BALANCE`); control accounts (1100/2000/1200/1500) reject direct postings unless `viaSubledger:true` (`CONTROL_ACCOUNT`); per-tenant presentation curation writes the RLS-scoped overlay only (`gl_coa`) and cannot mint/recode a master code (`COA_ADMIN_ONLY`) | Prev / Auto | GL-11 | `gl_coa` permission tests; control-account guard test; `compliance` GL-11 overlay-curation ToE (rename/group/active/order persist; `COA_ADMIN_ONLY`; non-`gl_coa` denied) |
...
| `COA_ADMIN_ONLY` | Overlay curation targeting a code not in the canonical master chart (attempt to mint/recode a master account) | Master-code create/recode is Admin/HQ via the canonical routes; curation can only rename/group/reorder/activate existing accounts |
| `OVERLAY_NO_TENANT` | Overlay curation called outside any tenant context (e.g. HQ/Admin with no tenant scope) | Curate within a company (tenant) context; the overlay is per-tenant |

## 14. Revision history

| Version | Date | Author | Summary |
|---|---|---|---|
| 1.9 | 2026-07-03 | Platform | **GL-11 — per-tenant Chart-of-Accounts curation (overlay write + UI).** Step 14 gains an overlay write-path: `PATCH /api/ledger/accounts/:code/overlay` (`ledger.service.ts updateAccountOverlay`, permission `gl_coa`, RLS-scoped to the caller's tenant) lets a finance user rename (EN/TH), set `group_label`, re-order (`sort_order`), and toggle `active` on their own tenant chart — writing only the `tenant_accounts` overlay (GL-10), never the canonical master universe (presentation-only; `?all=true` + postings unaffected). Curating a code absent from the canonical chart → `COA_ADMIN_ONLY` (master-code create/recode stays Admin/HQ on the canonical routes); called outside a tenant context → `OVERLAY_NO_TENANT`. `GET /api/ledger/accounts` gains `?include_inactive=true` so a `gl_coa` manager can see/re-activate curated-off rows. Web: the **ผังบัญชี** tab (`/accounting`) now shows `gl_coa`-gated affordances (edit dialog, group label, active toggle, ↑/↓ re-order) with a hint that master-code changes are HQ-only; the API error `code` is surfaced to the client so `COA_ADMIN_ONLY` maps to a friendly toast. No schema change (reuses `tenant_accounts`, migration 0139); no new RCM control (curation is a `gl_coa` affordance under the existing GL-10/GL-11 controls). ToE: new `compliance` GL-11 curation checks (rename/group + active off/on + `COA_ADMIN_ONLY` + non-`gl_coa` denied); `compliance` 128 / `basics` 215 green. User manual `06-general-ledger.md` §1 + UAT `05-general-ledger-close-uat.md` (UAT-UI-COA-03) + traceability matrix updated. |
