...
| Related RCM controls | PAY-01, PAY-02, PAY-03, PAY-06, ITGC-AC-19, GL-01; SoD R07 |
...
5. **Approval & disbursement.** On approval the Draft JE becomes **Posted** (now in balances) and the run records the approver/timestamp; the run is released for disbursement (bank file / payment) and the payslips are issued to employees. Each **สลิปเงินเดือน (payslip)** can be printed/emailed as a PDF from the shared document renderer, but because a slip carries PII the access is **PDPA-scoped (PAY-06)**: an employee downloads only **their own** slip via employee self-service (`GET /api/ess/payslips/:id/pdf`, perm `ess`) — the service resolves the caller to their own employee record from the JWT and a slip id that isn't theirs returns **404** — while HR/payroll (`GET /api/payroll/slips/:id/pdf` + `.../send-email`, gate `exec/users/creditors`) may print/email any slip in the tenant and an ess-only user is denied that route (403). The citizen ID is **masked to the last 4 digits** on the face of the slip (data-minimisation), on top of the field-level encryption at rest (ITGC-AC-19).
6. **GL posting (tenant-scoped).** Payroll expense, SSO/PIT liabilities, and net-pay payable post as a **balanced** journal entry to the GL. The leg accounts default to the standard literals (5600/5610/2350/2360 + PF 5620/2370, net pay Cr 1000) but a tenant may **re-map any role** via its `/setup/posting-rules` rows (`PAYROLL.GROSS` wages_expense, `PAYROLL.SSO` sso_expense/sso_payable, `PAYROLL.WHT` wht_payable, `PAYROLL.PF` pf_expense/pf_payable) — resolved as `override ?? literal` per run (docs/42 step 4; a typo'd account fails closed `INVALID_POSTING_ACCOUNT` at posting, GL-21). The rule change itself is **governed (GL-24, docs/43 PR-1)**: it lands PendingApproval and only a different user's approval activates it — an unapproved rule never touches a run. It lands as a **Draft at run time, effective only on approval** (**PAY-03**, **GL-01**; period controls per `04-general-ledger-close.md`). The run is **idempotent per (tenant, period)**: a PendingApproval or Posted run blocks a re-run (a Rejected one may be re-run). It is **scoped to a single tenant**: a tenant-bound user runs for their own tenant, while an HQ/Admin caller (whose request bypasses RLS) **must** name the tenant via `tenant_id` on `POST /api/payroll/runs` — otherwise the run is rejected `TENANT_REQUIRED`. The employee selection and the JE are filtered to that tenant so payroll can never consolidate employees of multiple tenants into one entry (**ITGC-AC-03**). A large run may be taken **off the request thread**: `POST /api/payroll/runs?async=1` enqueues the run as a background job (`background_jobs`, migration 0179) and returns `202 {job_id}` immediately; an in-process worker claims it (`FOR UPDATE SKIP LOCKED`) and executes the **same** `runPayroll` inside the job's own tenant transaction, so RLS scoping, the maker-checker Draft posting (PAY-03), and `TENANT_REQUIRED` are identical to the synchronous path. The progress is polled at `GET /api/jobs/:id` (RLS-scoped — a tenant sees only its own jobs). Because the run is idempotent per (tenant, period), the queue's at-least-once retry can never double-post. The synchronous call (no `?async`) is unchanged.
...
| 5 | An employee's payslip is exposed to someone other than the employee or authorised HR (a colleague pulls it by guessing an id; full citizen ID printed in the clear) | **PDPA-scoped payslip access**: self-service download resolves the caller to their OWN employee from the JWT + an `employee_id` predicate (a foreign slip id → 404); the HR print/email route is gated `exec/users/creditors` (ess-only user → 403); citizen ID **masked to last 4** on the slip (data-minimisation); RLS tenant-scoped | **Prev / Auto** | **PAY-06** | PDPA payslip-access test (own-200 / other-404 / HR-403) + masked-ID render |
...
|---|---|---|---|
| 0.7 | 2026-07-11 | Platform | **docs/43 PR-7 — PAY-02 tie-out widening (no control/flow change, no migration).** The payable roles flip Tier B→A: `PAYROLL.SSO.sso_payable` (2350), `PAYROLL.WHT.wht_payable` (2360) and `PAYROLL.PF.pf_payable` (2370) now accept GL-24-governed overrides, and the PAY-02 liability schedule + the remit endpoint read each liability's **WIDENED account set** `{canonical} ∪ {approved override}` (`LedgerService.postingAccountSet`; schedule lines expose `account_set`): the schedule still ties to the payrun accrual after an override, and treasury can remit against the override account — the outstanding check reads the SPECIFIC account being remitted (`NOT_LIABILITY_ACCOUNT` message widened). Closes the docs/42 latent gap (posting overridable but the schedule hard-pinned). ToE `payroll` 46 (4 PR-7 checks: widen role now accepts a rule → run posts 2351 → schedule set `{2350,2351}` reconciled → remit clears the override account exactly). See PN-06 for the PND3/53 + ภ.ธ.40 set widening; docs/43 rev 0.9. |
| 0.6 | 2026-07-11 | Platform | **GL-24 governance over the payroll posting-rule overrides (docs/43 PR-1).** Step 6: a `PAYROLL.*` override now lands PendingApproval and applies only after a DIFFERENT user approves (self-approve → `SOD_VIOLATION`); pinned roles (net-pay cash 1000) are rejected `OVERRIDE_ROLE_PINNED`; the payable roles stay Tier B (widen) until PR-7 extends the PAY-02 schedule set. ToE `payroll.ts` 42 (pending ignored → approve → re-mapped run → re-edit demotes). |
| 0.5 | 2026-07-11 | Platform | **Tenant posting-rule account overrides for the payroll JE (docs/42 step 4 — no control change, no migration).** Step 6: each payroll leg resolves `override ?? literal` from the tenant's ACTIVE `posting_rules` rows (`PAYROLL.GROSS/SSO/WHT/PF` roles, maintained on `/setup/posting-rules`); NULL-tenant demo rows never apply, so an un-configured tenant posts byte-identically. A typo'd override fails closed at `postEntry`'s new account-universe guard (`INVALID_POSTING_ACCOUNT`, GL-21). ToE: `payroll` harness — a tenant `PAYROLL.GROSS`→5601 rule re-maps the salaries debit end-to-end (37 checks green). |
...
| 0.7 | 2026-07-05 | Platform | **PAY-06 — printable/emailable payslip with PDPA-scoped access.** Step 5: the สลิปเงินเดือน is now a first-class printable/emailable document (shared HTML→PDF renderer, HTML fallback when Chromium absent) via a new `PayslipPdfService`. Access is PDPA-scoped: an employee downloads only their OWN slip through self-service (`GET /api/ess/payslips/:id/pdf`, perm `ess`) — the service resolves the caller's employee from the JWT and ANDs an `employee_id` predicate so a foreign slip id → **404**; HR/payroll (`GET /api/payroll/slips/:id/pdf` + `POST .../send-email`, gate `exec/users/creditors`) prints/emails any slip in the tenant while an ess-only user is denied (403). The citizen ID is **masked to the last 4 digits** on the face of the slip (data-minimisation, on top of ITGC-AC-19 encryption at rest). New RCM control **PAY-06** (RCM now 183; 180 Implemented). No migration (read-only over existing `payslips`/`payruns`/`employees`). ToE: `payroll` harness (own-200 / colleague-404 / HR-200 / ess-only-403 / masked-ID / email path 503 EMAIL_NOT_CONFIGURED). Manual `08-payroll.md` + UAT `07-payroll-uat.md` updated. |
...
| 0.7 | 2026-07-02 | Platform | **Module consolidation (docs/28 PR #2) — code pointer only.** `modules/tax-reports` → `modules/tax/reports/`; no behavior change. |
| 0.8 | 2026-07-07 | Platform | **Employee-master fields exposed on `/payroll` (master-data audit Phase 2 — no control change, no migration).** `department`/`hourly_rate`/`pf_rate` were **accepted at employee creation but missing from `PayrollService.fmtEmp()`** — so once saved they were never returned to any screen; fixed. `sso_no`/`allowances`/`sso_eligible`/`bank_account`/`start_date` were already accepted by `EmployeeDto` and returned by `fmtEmp()`, but the **add-employee form never collected them** — the "เพิ่มพนักงาน" form on `/payroll` now does, and the employee table gains **แผนก**/**วันที่เริ่มงาน** columns. There is still **no employee-edit endpoint** (create + list only) — an employee's fields can only be set at creation; adding update capability is out of scope for this pass (flagged for a future phase). `manager_id`/reports-to hierarchy, termination-date history, and `employment_type` remain genuinely **absent from the schema** (not merely hidden) — also out of scope here (Phase 2 exposes existing columns; new columns are Phase 3 territory). ToE: `payroll` harness **36** (create with the full field set; list projects department/hourly_rate/pf_rate). Manual `08-payroll.md` updated. |
