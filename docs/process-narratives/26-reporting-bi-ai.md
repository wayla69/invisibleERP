...
| 1.4 | 2026-07-02 | Platform | **Semantic RAG provider (docs/27 R4-1 / AUD-AI-01).** `EmbedderService` gains a real semantic provider (`EMBED_PROVIDER=voyage`, plain-fetch adapter, 15s bound) behind the SAME production DPA gate as the AI features, with fail-safe degradation to the local lexical embedder (+ throttled ops alert). New `kb_chunks.embed_provider` (migration 0219): every chunk records its embedding space, search compares only within the query's space, and `POST /api/ai/kb/reembed` (ingest authority) migrates the corpus after a provider switch. ToE: `rag` harness 11 (space stamping; foreign-space chunk excluded until re-embedded; reembed restores) + `embedder` unit (fallback never throws/mislabels). DPA sub-processor register gains Voyage. |
