...
| BAD_DIMENSION (400) | Segment profitability requested for an unknown dimension | Reject; use `by=branch` / `cost_center` / `project` (step 3d). |
...
| 2.2 | 2026-07-05 | Platform | **Segment profitability (docs/35 Phase 5, no migration, no new control).** §7 — added step 3d: `GET /api/finance/metrics/profitability?by=branch\|cost_center\|project` returns the P&L by an accounting dimension already on the postings (per-segment revenue/COGS/gross/opex/net + margins + contribution %, sorted, reconciles to the consolidated P&L; untagged ⇒ "Unassigned"; `BAD_DIMENSION` on a bad dim; empty period ⇒ zero not error). Surfaced on `/finance/profitability` (dimension switcher + net-contribution bars + P&L matrix). §13 — `BAD_DIMENSION` error code. Read-only aggregation — posts nothing; reinforces GL-13 (dimensional completeness) + ELC-07 (analytical review), **no new RCM control**. Customer/product profitability (sub-ledger, not a GL dimension) is a documented follow-up. ToE: `finance-kpi` harness **54** (branch segments reconcile to P&L rev 150k/net 50k; BR1 net-margin 66.67% / contribution 40%; empty period → 0 segments; bad dim → 400; non-finance role denied). Manual `09 §1e`; UAT `09` UAT-UI-PROF-01. |
| 2.1 | 2026-07-05 | Platform | **CFO Command Center dashboard (docs/35 Phase 2, no migration).** §7 step 3c + §9 — the finance KPI pack now has a web home: `/finance/command-center` (server-prefetch page + client island) renders it as a RAG scorecard grouped by KPI family — value + RAG (icon+label) + prior-period/prior-year/budget deltas per tile, expandable to a lazy 12-month sparkline, drill to the statements. Live off the SSE bus: `BiService.refreshSnapshot` now also publishes `fin_kpi_refresh` (compact finance headline + red-flag count) and the client re-pulls on it (reuses `useRealtime`). Nav entry (Finance → `nav.command_center`) + i18n (`fnx.cfo.*`). Read-only; no GL/SoD change; control ELC-07 unchanged. ToE: web build + `pnpm -r typecheck` clean; `bi` harness green (refreshSnapshot still passes). Manual 09 §1b + UAT (UAT-UI-CFO-01) updated. |
