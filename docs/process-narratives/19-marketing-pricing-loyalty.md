...
26. **LINE LIFF member login (CRM Phase 4).** Besides phone-OTP, a member can log in through **LINE**: `POST /api/member/auth/line` (`@Public`, `@NoTx`) verifies the LIFF **idToken** against LINE (production, using `LINE_CHANNEL_ID`) — a dev id is accepted only outside production — and mints the **same** member token for the member whose `pos_members.line_user_id` matches **within the requested tenant** (`LINE_NOT_LINKED` otherwise). A logged-in member links their LINE account via `POST /api/member/link-line`, with a one-LINE-↔-one-member-per-tenant partial unique (`LINE_ALREADY_LINKED`). *Control: ITGC-AC — LINE login is signature-verified server-side; the dev bypass cannot run in production.*
