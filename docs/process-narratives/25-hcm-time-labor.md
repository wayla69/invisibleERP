...
| 0.3 | 2026-06-24 | Platform | Expense approval now **raises an AP reimbursement payable** (vat-exempt, via `FinanceService.createApTxn`, acct 5100; `expense_claims.ap_txn_no`, migration 0106) instead of a bare GL post — so the reimbursement appears in **AP aging**, settles through the **AP pay flow**, and keeps the AP sub-ledger ↔ GL control account (2000) reconciled (REC-01). SoD self-approval guard unchanged. Verified by the `ess` + `basics` harnesses. |
