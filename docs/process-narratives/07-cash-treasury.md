...
| 2 | Captured funds unrecorded (orphan), or funds booked that did not move | Persist Pending before capture; Failed on error; **real PSP charge** for card/e-wallet — no Captured without an actual charge | Prev / Auto | REV-03 | Negative-path + `payments-gateway` harness (decline → not Captured) |
