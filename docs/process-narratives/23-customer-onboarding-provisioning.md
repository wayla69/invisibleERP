...
0. **Onboarding trigger (production = god-only).** In production the request originates on the **god path**: a platform owner either provisions directly (`POST /api/admin/tenants`), issues a single-use **invite** the prospect redeems, or **approves** a public **request-access** entry (`POST /api/auth/signup-requests` files a pending request; `GET /api/admin/signup-requests` reviews it; `…/:id/approve` provisions with the requester's chosen — hashed — password). Public `POST /api/auth/signup` self-serve provisioning is **off in production** (`isSignupAllowed()` prod-false; `PUBLIC_SIGNUP_ENABLED` no-op) and runs only in dev/harness. Whichever god-authorised path fires, provisioning then proceeds atomically as below (**ITGC-AC-18**). A request filed from the public **/plans** configurator additionally carries the prospect's **pack selection** (`requested_plan` — stored as the PACK_TO_PLAN-mapped real plan code — plus `requested_interval` and recognised `requested_addons`; migration 0451): the Platform Console onboarding queue shows it (แพ็กเกจที่ขอ column) and **approve provisions that pack** (plan + billing interval + add-ons stamped on the trialing subscription) instead of the legacy `free` default. Advisory only — junk values are dropped, never rejected, and the approver can still change the plan afterwards (`POST /api/admin/tenants/:id/plan` / `/addons`). **The decision is notified to the requester by email (A1, migration 0452):** approve → `signup_approved` (login link, pinned to the provisioned tenant), reject → `signup_rejected` (with the reviewer's reason), and an invite issued **with an email address** → `signup_invite` (the single-use link). Every send is recorded first in the **`platform_emails` outbox** (Queued → Sent/Failed with provider evidence — the audit trail for platform mail) and delivered by the background job worker (`platform_email` job, retry/backoff) or the god sweep `POST /api/admin/emails/deliver-pending`; the outbox is god-only (`GET /api/admin/emails`). Unset `MAIL_PROVIDER` = a no-network mock (dev/harness); `resend`/`postmark` send for real. **An email failure never blocks or reverses the decision** (fire-and-forget enqueue). ToE: `cutover/onboarding.ts` §3d-bis (165 checks).
...
10-bis. **Post-onboarding lifecycle automation (A2, migration 0453).** After provisioning, the daily **`saas_lifecycle`** job (BI scheduler action job; god can also run it on demand via `POST /api/admin/saas-lifecycle/run`) operates the subscription lifecycle without manual chasing: **Trialing** → `trial_reminder` emails at T-7 and T-1 (keyed to the trial END date, so an extended trial re-arms them); an expired **paid** trial gets a grace window (`SAAS_TRIAL_GRACE_DAYS`, default 7) then is **auto-suspended** (`suspended_by = 'saas_lifecycle (auto)'`, `company_suspended` email + god-inbox event); an expired **฿0** plan simply activates (enterprise-custom additionally flags the god inbox for sales follow-up). **PastDue** walks a dunning ladder (`payment_failed` email at detection, day ≥7, day ≥14; auto-suspend at ≥ `SAAS_PASTDUE_SUSPEND_DAYS`, default 21); recovery to Active closes the cycle so a later PastDue restarts fresh. Every side effect is anchored on an idempotent **`saas_lifecycle_events`** row (unique dedup key — insert-or-skip is the lock), so overlapping schedules or manual sweeps can never double-remind or double-suspend; the ledger is god-readable at `GET /api/admin/saas-lifecycle/events`. Emails ride the A1 outbox (step 0). ToE: `cutover/onboarding.ts` §3g-bis (13 checks) + vitest `saas-lifecycle.test.ts` (7, pure planner).
...
|---|---|---|---|
| 1.0 | 2026-07-21 | Platform | **A2 — SaaS lifecycle automation (migration 0453).** Daily `saas_lifecycle` job: trial T-7/T-1 reminders, expired-paid-trial grace → auto-suspend (attributed `saas_lifecycle (auto)`), ฿0-plan activation, PastDue dunning ladder (detect/7d/14d → suspend ≥21d), recovery clears the cycle; idempotent via the `saas_lifecycle_events` dedup ledger; god run/feed endpoints; emails via the A1 outbox. New §7 step 10-bis. **No new numbered control** (automates the existing suspend authority with full attribution + evidence). ToE: `onboarding.ts` 165→178 + vitest `saas-lifecycle.test.ts` (7). |
| 0.9 | 2026-07-21 | Platform | **A1 — onboarding decisions notify the requester by email (migration 0452).** Approve/reject/invite-with-email each queue a th/en transactional email (`signup_approved`/`signup_rejected`/`signup_invite`) into the new platform-level **`platform_emails` outbox** (about_tenant_id convention, no RLS), delivered via the background job queue (mock provider without `MAIL_PROVIDER`; resend/postmark via fetch — no new deps) or the god deliver-pending sweep; god-only outbox read `GET /api/admin/emails`. Fire-and-forget: an email failure never blocks the decision. New §7 step-0 notification note. **No new numbered control** (operational evidence for ITGC-AC-18 decisions). ToE: `cutover/onboarding.ts` 160→165 + vitest `mailer-templates.test.ts` (5). |
| 0.8 | 2026-07-20 | Platform | **/plans pack carry-through (migration `0451`, no new control).** The public request-access form forwards the prospect's /plans pack selection; the request row stores the mapped real plan code + interval + add-on keys, the console onboarding queue displays them, and approve honours them at provisioning (see §7 step 0). Strengthens the ITGC-AC-18 approve flow's fidelity to the commercial intent; the god approval remains the sole provisioning authority. ToE `cutover/onboarding.ts` 157→160. |
