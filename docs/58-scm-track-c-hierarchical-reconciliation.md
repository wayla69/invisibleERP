...
| **C3** | **Optimal reconciliation (MinT).** Engine adds `mint` with `ols` / `wls_struct` / `wls_var` / `shrink` covariance (Schäfer–Strimmer); residual-history plumbing from extract; backtest proving MinT ≤ base error on a synthetic hierarchy. | — | **M** |
| **C4** | **Coherent probabilistic paths.** `reconcile_paths` projects the sample paths through `P=S·G` (clip+renormalize); reconciled scenarios reach `/v1/optimize`; path-coherence pytest + harness invariant. | — | **M** |
