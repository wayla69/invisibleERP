...
- Conversion candidates, in order of bundle-weight × read-share: `accounting/page.tsx` ✅ **(#1,
  ...
  island)**, `tax/reports` ✅ **(#4, 2026-07-02 — the RFC's "reports": the statutory VAT/ภ.ง.ด. views;
  prefetch pinned to the island's default period so it lands on the initial queryKey)**, `insights` ✅
  **(#5, 2026-07-02 — overview summary prefetched)**. **Top-5 list complete.** Note: a shell+island conversion is **count-neutral**
...
- ~~Ratchet idea (follow-up)~~ **Ratchet ARMED (2026-07-02, after conversions #1+#2 per this gate):**
  ...
  `'use client'` against `use-client-baseline.json` (232 at present — re-based once to 233 when PR #320's new
...
| 1.9 | 2026-07-02 | Platform | **RSC conversions #4 (tax/reports) + #5 (insights) shipped — the §4 top-5 list is COMPLETE.** Both use the proven shell+island pattern: tax/reports prefetches `/api/tax-reports/output-vat` for the island's default period (guarded so a period change fetches fresh), insights prefetches `/api/analytics/dashboard-summary`. No standalone `/reports` route exists — the RFC's "reports" resolved to the statutory tax/reports page (299 lines, read-heavy) over the 127-line scheduled-reports config UI. Web build + e2e 15/15 + use-client 233 flat. AUD-ARC-09's conversion program is delivered; the ratchet carries the direction forward. |
