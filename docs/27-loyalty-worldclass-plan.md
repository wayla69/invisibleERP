...
> **Date:** 2026-07-02 · **Status:** v1.3 — **DELIVERED** (W1 + W2 in #310, W3 shipped) · **Owner:** ERP / Product (CMO + SVP-IT review)
...
|---|---|---|---|
| 1.3 | 2026-07-02 | Platform | **W3 SHIPPED — plan DELIVERED.** NPS closed loop: `nps_responses` (migration `0223`), tokenized single-use public `GET/POST /api/nps/:token` (atomic answer guard, 7-day expiry, no PII in the URL), detractor ≤6 fires `loyalty.nps_detractor` into webhooks + automation, member-360 flag + `GET /api/nps/summary` trend, triggers `/api/nps/send`·`/send-due`·BI job `nps_post_purchase`, public answer page `/nps/[token]`. Messaging governance: OPT-IN per-tenant quiet hours + global cross-channel weekly marketing cap enforced inside `MessagingService.send` (transactional exempt by campaign base; audited `quiet hours`/`global cap` skips; journeys re-arm the deferred step — claim-first-safe); config `GET/PUT /api/messaging/governance` + `/settings/messaging` card (a `governance` row in `tenant_messaging_config` — deviation from the plan's default-on: defaults stay OFF until saved, for deterministic CI + unchanged existing behaviour, with 21:00–09:00 / 4 suggested in the UI). No new control (rides MKT-04/12). Harness crm +5 → 53/53, line-crm +3 → 29/29; narrative 19 rev 1.35 (steps 31–32), manual 13 rev 1.31 (§7c), UAT 56→58. |
