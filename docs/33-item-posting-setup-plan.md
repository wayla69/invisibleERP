...
> **Date:** 2026-07-04 · **Status:** v1.3 — PR1–PR4 DELIVERED (warehouse account defaults = tracked follow-up) · **Owner:** ERP / Finance
...
| 2026-07-04 | 1.3 | PR4 delivered: `TaxJobsModule` scheduled tax automation — `tax_wht_cert_batch` auto-issues the 50-ทวิ from AP-payment WHT (labour/service withholding, idempotent per `payment_no`, closes the TAX-03 gap), `tax_pp30_draft`/`tax_pnd_draft` register period filing drafts (TAX-05), `tax_remittance_reminder` for deadlines. RCM TAX-03/TAX-05 refreshed (181). `taxdocs` 58 green; `basics` 230, `compliance` 134 green. **All four PRs delivered.** Follow-up: warehouse (`locations`) account-default columns + a project/manufacturing internal-labour WHT posting if that scope is wanted. | ERP/Finance |
