...
| 2026-07-04 | 1.7 | Follow-up — **bulk Excel/CSV import-export on the setup screens**: `/setup/item-categories` and `/setup/tax-codes` now carry the same Export / Template / Import panel as the Master Data hub, backed by new entity-scoped `/api/item-setup/io/*` endpoints (reuse the `masterdata` registry engine; gated to the setup duties `md_item`/`md_config`/`masterdata`/`exec` and allow-listed to those two entities, so a narrow role gets the bulk surface without the coarse `masterdata` duty — SoD **R13** preserved). The importer also now accepts a direct `.xlsx` upload (base64, parsed server-side) alongside CSV, so a downloaded template round-trips without a Save-As-CSV step. Shared web `MasterIo` component. `ext` +6 checks (277). Docs: PN-17 §7.3b/3c + rev 0.6, user-manual/11, UAT-ADM-038/039. | ERP/Finance |
