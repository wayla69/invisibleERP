...
| **Customer messaging (LINE / SMS / email)** | `modules/messaging/gateways.ts` (+ `tenant-messaging.service.ts`) | mock records as `sent` | **per-tenant** `PUT /api/messaging/providers/:channel` → else env `LINE_CHANNEL_TOKEN` · `SMS_API_KEY`+`SMS_API_URL` · `SMTP_HOST` | All channels implemented: LINE push + **OA broadcast**, SMS (provider-agnostic HTTP REST + `SMS_SENDER`), email (SMTP/nodemailer). **Resolution order: per-tenant creds (encrypted at rest) → platform env → mock**, so a tenant can use its own LINE OA / SMS sender / SMTP mailbox. |
