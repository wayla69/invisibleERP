...
| **Customer messaging (LINE / SMS / email)** | `modules/messaging/gateways.ts` | mock records as `sent` | `LINE_CHANNEL_TOKEN` (LINE push + OA broadcast) · `SMS_API_KEY`+`SMS_API_URL` (SMS) · `SMTP_HOST` (email) | All channels implemented: LINE push + **OA broadcast** (`/message/broadcast` → all followers), SMS (provider-agnostic HTTP REST + optional `SMS_SENDER`), email (SMTP via nodemailer — `SMTP_PORT`/`USER`/`PASS`/`FROM`/`SECURE`/`SUBJECT`). Each activates when its env is set, else falls to mock. |
