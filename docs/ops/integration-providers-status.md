...
| **Customer messaging (LINE / SMS / email)** | `modules/messaging/gateways.ts` | mock records as `sent` | `LINE_CHANNEL_TOKEN` (LINE push — real today) · `SMS_API_KEY` (SMS) · `SMTP_HOST` (email) | LINE push is fully implemented; SMS/email use the same shape pending a provider account. |
