...
| 1.3 | 2026-07-02 | Platform / SRE | New ops alert **`realtime_redis_publish_failed`** (docs/24 R1-3): Redis pub/sub behind the SSE buses failed a publish — the event was delivered to same-node clients only; other replicas missed it. Respond: check `REALTIME_REDIS_URL` connectivity / the Redis add-on; alert is throttled to 1/min. |
