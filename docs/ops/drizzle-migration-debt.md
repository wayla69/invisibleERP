...
| 2026-07-03 | v1.3 | Platform / DB | **Production drift surfaced as a boot outage.** Every prod deploy 2026-07-02→03 died at boot-time `drizzle-kit migrate`: `0218_tenant_indexes_backfill` hit `42P01 relation "tip_distribution_lines" does not exist` — prod's DB is missing the `0146_tip_distribution` tables (a live casualty of the historical dup-number/renumber era; fresh DBs are unaffected, so every harness stayed green while only prod broke — and the crash line was invisible from CI because `railway logs` returns only the ACTIVE deployment; a boot-crashed one never activates). Fixes: (a) `0218` rewritten as a table-existence-guarded DO loop (`to_regclass`) — identical effect where all tables exist (the `tenant-idx` harness still proves full coverage in CI), skip-not-crash where prod's catalog has gaps; editing was safe because prod had never applied it and applied-elsewhere runs are tracked by tag, not content; (b) new `0230_prod_repair_tip_tables` re-runs 0146's idempotent DDL + tenant indexes + RLS loop to heal prod. **Lesson recorded:** a drifted prod catalog turns any "touch every table" migration into an outage — bulk migrations must be existence-guarded, and prod's applied-migration ledger should be reconciled against the journal (follow-up: dump `__drizzle_migrations` from prod and diff; more 0146-era gaps may exist beyond tips). Diagnosis artifacts: `ops-railway-diag.yml` (fetches FAILED-deployment logs by ID via GraphQL). |
