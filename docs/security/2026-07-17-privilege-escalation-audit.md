...
|---|---------|-------|----------|----------------------|
| **PE-1** | `POST /api/platform/api-keys` writes `scopes` verbatim — no cap to the minter's perms → AccessAdmin mints a key with `gl_post`/`creditors`/… scopes | Vertical / SoD-bypass | **High** | ✅ **Fixed** (this branch — scopes that map to an internal permission bounded to the minter, `403 KEY_SCOPE_EXCEEDS_GRANTOR`; public-API-only scopes like `catalog:read` exempt) |
...
| **PE-3** | `GET /api/hcm/leave/balances` is `ess`-reachable but ignores the caller → any employee reads a colleague's (or **all**) leave balances | Horizontal / BOLA | **Medium** | ✅ **Fixed** (this branch — non-HR `ess` callers own-scoped via `callerEmpCode`) |
| **PE-4** | `POST /api/quality/inspect` (disposition=Scrap) posts an immediate GL write-off gated only by `warehouse`/`bom_master`, **no maker-checker** | Vertical / self-approval | **Medium** | ✅ **Fixed** (this branch — Scrap requires `quality_approve`/`exec`, `403 SCRAP_APPROVAL_REQUIRED`) |
| **PE-5** | `PUT /api/procurement/match/tolerance` gated `creditors` only, no maker-checker → the AP beneficiary widens 3-way-match tolerances and defeats the EXP-01 hold | Vertical / SoD | **Medium** | ✅ **Fixed** (this branch — gated `exec`/`approvals`, AP beneficiary excluded) |
| **PE-6** | `POST /api/platform/mfa/setup` overwrites the TOTP secret + sets `mfaEnabled:false` with no already-enrolled guard / no step-up (the `auth` surface blocks exactly this) | Auth-control bypass | **Medium** | ✅ **Fixed** (this branch — refuses re-enrol when enabled, `400 MFA_ALREADY_ENABLED`) |
| **PE-7** | Class of controllers pass body `tenant_id` straight to the service **without** the Admin gate that `ledger`/`leases` use (deferred-tax, fx-reval, tax-utp, treasury-pool/-debt/-hedge/-invest) | Horizontal (mitigated) | **Low** | ✅ **Fixed** (this branch — non-Admin `tenant_id` nulled at the controller, `u.role==='Admin' ? … : null`) |
| **PE-8** | Portal `deleteCustomer/Supplier/PurchaseOrder` issue the final `DELETE` without the `AND tenant_id` predicate (scoped SELECT + RLS still protect it) | Horizontal (defense-in-depth) | **Low** | ✅ **Fixed** (this branch — tenant predicate on the DELETE) |
| **PE-9** | Portal sub-user allow-list includes `loyalty`; via OR-gating it satisfies the member points-transfer gate, sidestepping the `crm_points_adjust` SoD split | Vertical / SoD | **Low** | ✅ **Fixed** (this branch — `loyalty` removed from the portal sub-user allow-list) |
| **PE-10** | `POST /api/inventory/waste` gated `warehouse`/`pos`/`order_mgt` → a sales role posts a non-perpetual stock write-off (perpetual items are hard-blocked to INV-07) | Vertical | **Low** | ✅ **Fixed** (this branch — gated `wh_adjust`/`exec`, sales duties dropped) |
| **PE-11** | `POST /api/costing/allocate` admits `cust_pos`/`order_cust` → a portal principal creates/releases stock reservations | Vertical | **Low** | ✅ **Fixed** (this branch — `cust_pos` dropped from allocate/release/fulfill; read-only `atp/check` unchanged) |
...
|-----|------|--------|--------|
| 1.1 | 2026-07-17 | Security review workstream | **All 11 findings remediated on `claude/privilege-escalation-api-test-vqvytp`** (6 doc-synced PRs, fail-closed). PE-1/PE-2 (bound key scopes + escalating grants to the grantor; escalations routed to the existing two-person queue — PE-2 approach chosen by the owner), PE-3 (leave-balance own-scope), PE-4/PE-5 (scrap write-off + match-tolerance segregation), PE-6 (platform MFA re-enrol block), PE-7/PE-8 (server-derived `tenant_id` + portal DELETE predicate), PE-9/PE-10/PE-11 (scope-breadth tightening). Each carries a negative harness test and a narrative revision (08-itgc §2.9–2.12, PN-02 1.9, PN-15 0.8, PN-29 0.2). Regression suite green (onboarding 153, ext 306, basics 458, compliance 179, match 86, mfg-depth 13, hcm-leave 19, gaps 36, waste 20, costing 20, treasury/tax/fx). No new RCM control. |
