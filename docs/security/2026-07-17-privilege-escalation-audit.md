...
|---|---------|-------|----------|----------------------|
| **PE-1** | `POST /api/platform/api-keys` writes `scopes` verbatim — no cap to the minter's perms, **no SoD check** → AccessAdmin mints a key with `gl_post`/`creditors`/… scopes | Vertical / SoD-bypass | **High** | Open (residual of P-theme; key path not examined) |
| **PE-2** | `admin-users` grant bounds only the **Admin role** + SoD *pairs*; no `granted ⊆ caller` check → AccessAdmin grants a single transactional duty (e.g. `gl_post`) to a puppet account | Vertical | **High** | Open (residual of P1/P2 theme) |
