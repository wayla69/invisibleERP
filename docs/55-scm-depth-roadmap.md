...
| **B4** ✅ | **DRP roll-up.** Time-phased net requirements roll branch→DC→supplier and hand off to the **existing** procurement PR flow (reuse, do not rebuild). `scm-drp.service.ts` + time-phased convert. | — | L |
...
| 0.15 | 2026-07-23 | Supply-chain / Planning | **B4 delivered — Track B COMPLETE (DRP time-phased roll-up)** (docs/57): new pure `scm-drp.service.ts` — lot-for-lot MRP netting per node (net period gross requirements against projected on-hand + in-transit, lot-size to the inbound lane moq/pack, offset the receipt back by the lane lead → planned release), rolled bottom-up so branch releases become the DC's gross requirements and the DC's releases become the supplier-facing requirements. `ScmNetworkRunService.applyDrp` time-phases every node plan's `orders`; `ScmNetworkPlanService.convertPlan` rolls the DC's supplier-facing releases up to a PR with one line per release (each with its own required date) via the existing `ProcurementService.createPr` seam (idempotent by `pr_no`). **No new control, no migration, no engine change, no RCM change, contract unchanged (v2).** `scm` harness **79/79** (+6 B4, `scm-mfg` shard). PN-34 §7.21, manual ch.21, UAT §20 (UAT-SCM-074..077), traceability. **Track B (B1→B4) fully delivered; only Track D · D4 (SCM-07 accuracy monitoring) remains across the whole docs/55 roadmap.** |
