...
| **D4** ✅ | **Accuracy monitoring.** Track WAPE/bias per item over time, drift alerts, force-refit feedback — surfaced on the BI live SSE bus + a `scm_forecast_accuracy` report. `scm-accuracy.service.ts` + `scm_accuracy_history` (migration 0479). | **SCM-07** (forecast-accuracy detective control) | M |
...
| 0.16 | 2026-07-23 | Supply-chain / Planning | **D4 delivered — Track D COMPLETE + the ENTIRE docs/55 SCM depth roadmap DELIVERED** (docs/59). Forecast-accuracy monitoring (control **SCM-07**, detective): `scm-accuracy.service.ts` reconciles the realized WAPE/bias per (branch, item) vs the fit-time baseline into `scm_accuracy_history` (**migration 0479**, canonical 0232-form RLS); a sustained degradation (realized WAPE > baseline × `accuracy_degradation_factor` for `accuracy_sustained_periods` consecutive as-of dates — additive `scm_settings` columns + env fleet-defaults) flags the series `degraded`, raises `captureOpsAlert('scm_forecast_degraded')` + a **`scm_accuracy_degraded`** SSE event, and force-refits it on the next batch (D2 loop closed). Schedulable `scm_accuracy_refresh` job + read-only `scm_forecast_accuracy` BI report. No wire/contract change (v2), no GL, no golden-master path. RCM control **SCM-07** (total 315→316, implemented 312→313). `scm` harness **84/84** (+5 D4). PN-34 §7.22, manual ch.21 §2, UAT §21 (UAT-SCM-078..082), traceability, `.env.example`/deployment.md. **All four tracks now delivered — A (promo/price/elasticity/analog A1–A4), B (multi-echelon B1–B4), C (hierarchical reconciliation C1–C4), D (scale-out + accuracy D1–D4).** |
