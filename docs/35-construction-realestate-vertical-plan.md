...
> **Date:** 2026-07-05 · **Status:** v0.11 — **P0–P5 + depth follow-ups D1–D5 + certificate/tax-invoice documents DELIVERED** — the vertical is **feature-complete**: contractor lifecycle + real-estate developer incl. ownership transfer, tax (VAT/WHT in+out) & rev-rec correct, UI-complete, with scheduled sweeps and printable/emailable Thai documents (progress-claim tax invoice, subcontract valuation certificate) · **Owner:** ERP / Product
>
...
### A1 — Progress-claim valuation & certificate (control **PROJ-16**)
...
- **Maker-checker** certification (`proj_billing` raises, `proj_billing_certify`/`gl_close` certifies; SoD
  pair) → **PROJ-16**: work cannot be billed beyond certified % and retention is withheld per policy.
...
### B1 — Subcontract order against BoQ scope (control **PROJ-17**)
...
- `subcontract_valuations` (per period: certified % / value this period, previous, retention withheld,
  ...
  certifies; SoD) → **PROJ-17**: no valuation beyond certified progress; retention withheld per policy;
...
### C1 — Estimate / draft BoQ on the opportunity (control **PROJ-18**)
...
- **On win → award:** `POST /api/tenders/:no/award` **seeds the project + an approved BoQ** from the tender
  lines (bid_rate → BoQ `rate`) in one step — the missing bridge. Governed by **PROJ-18** (award is an
...
|---|---|---|
| **PROJ-16** | Preventive | Progress-claim certification & retention — no billing beyond certified % to date; retention withheld per policy; maker-checker (raise ≠ certify). |
| **PROJ-17** | Preventive | Subcontract valuation & retention payable — no valuation beyond certified progress; back-charges & retention audited; certified net to AP; maker-checker. |
| **PROJ-18** | Preventive | Tender→award governance — award is authorised; the seeded project BoQ enters draft→approved (controlled budget baseline). |
...
RCM would reach **~187** (PROJ-16/16/17 + RE-01..04). Narratives affected: **PN-16** (project-accounting —
...
|---|---|---|---|---|
| **P0** ✅ **DELIVERED** | Shared **retention sub-ledger** + retention GL accounts (1170/2440) | 0249 | (structure; feeds PROJ-16/16) | `projects` 165→**178** — withhold/release, over-release guard, due worklist, SCF operating-bucket |
| **P1** ✅ **DELIVERED** | **Track A** progress billing (งวดงาน) + retention receivable | 0250 | **PROJ-16** | `projects` 178→**190** — BoQ-line cumulative valuation, certify SoD, retention withheld to sub-ledger, AR-net/retention/revenue JE, Fixed-contract cap |
| **P2** ✅ **DELIVERED** | **Track B** subcontracts + retention payable + back-charge | 0251 | **PROJ-17** | `projects` 190→**203** — subcontract BoQ commitment, valuation certify SoD, retention withheld payable, AP-net/WIP/retention JE, back-charge, over-budget cap |
| **P3** ✅ **DELIVERED** | **Track C** tender/estimate → award seeds project+BoQ | 0252 | **PROJ-18** | `projects` 203→**214** — estimate cost build-up, status machine, won-only idempotent award → Fixed project + draft BoQ (bid→rate), maker-checker approve baseline, win-rate |
...
| **P5** ✅ **DELIVERED** | **Track D3** ownership transfer + revenue recognition (D4 workspace shipped in Depth-4) | 0255 | **RE-04** | `projects` — settled-only authorised transfer, revenue rec (2410→4200) + unit-cost relief (5800←1200), NOT_FULLY_SETTLED guard |
...
- UAT under `docs/uat/` from **UAT-O2C-243+** (contractor) and a new **UAT-RE-01+** family (property), each
  ...
  and keep the traceability matrix in sync (map to PROJ-16/16/17, RE-01..04).
...
|---|---|---|---|
| 0.11 | 2026-07-05 | ERP / Product | **Certificate / tax-invoice documents DELIVERED (presentation layer).** Printable, bilingual (Thai primary) A4 documents for the vertical, on the shared `doc-html`/`PdfRenderer`/`DocEmailService` spine (HTML→PDF, HTML fallback when Chromium absent). **(1) ใบวางบิลงวดงาน / ใบกำกับภาษี** (progress-claim tax invoice) — `GET /api/progress-billing/:claimNo/pdf` + `POST /api/progress-billing/:claimNo/send-email`; BoQ-line movement (% / cumulative / prev / this-claim), gross → retention → net → VAT → AR total, baht-in-words. **(2) ใบรับรองผลงานผู้รับเหมาช่วง** (subcontract valuation certificate) — `GET /api/subcontracts/valuations/:valNo/pdf` + `POST .../send-email`; scope, gross → retention → back-charge → net → WHT (ภ.ง.ด.53) → input VAT → AP payable, baht-in-words. Both renderers `@Optional`-injected (harnesses still build); Print buttons added to `/projects/billing` + `/projects/subcontracts` (`doc.print_pdf`). **No new control / permission / GL — presentation-only, RCM stays 187.** Docs-synced: PN-16 rev 0.43, user-manual 14 rev 2.22. `projects` harness 240→**242** (both documents render); `basics`/`compliance`/`tenant-idx`/`migration-parity`/`rcm-census`/`ts-debt`/`use-client` all green; API + web build + typecheck clean. |
| 0.10 | 2026-07-05 | ERP / Product | **P5 (RE-04 ownership transfer) + subcontractor input VAT DELIVERED — the vertical is feature-complete.** **RE-04:** `POST /api/realestate/contracts/:no/transfer` — an authorised (`re_transfer`), **fully-settled-only** (`NOT_FULLY_SETTLED`/`CONTRACT_NOT_ACTIVE`), idempotent act that recognises the deferred sale revenue (**Dr 2410 / Cr 4200**) and relieves the unit's construction cost (**Dr 5800 / Cr 1200**), flipping unit + contract to `transferred` (new `re_units.cost`, `re_contracts.transfer_entry_no/transferred_at`, migration 0255). **Input VAT:** a subcontract carries a `vat_pct` → the certified valuation books recoverable **input VAT (Dr 1300 ภาษีซื้อ, new COA account)**, so AP = net − WHT + VAT (balanced). New control **RE-04** → RCM **187**; PROJ-17 amended (input VAT). Docs-synced: PN-31 rev 0.2, PN-16 rev 0.42, user-manual 14 rev 2.21 + 15 rev 1.1, UAT-RE-04 + traceability. `projects` harness 238→**240**; `basics` 251 / `compliance` 134 / `tenant-idx` / `migration-parity` / `rcm-census` (190/187) / `ts-debt` all green; API + web typecheck clean. |
| 0.9 | 2026-07-05 | ERP / Product | **Depth follow-ups D1–D4 DELIVERED** (post-P4 audit — the vertical was "architecturally deep, functionally ~60%"). **D1:** retention *release* posts GL (customer 1170→1100, subcontractor 2440→2000) + a `retention_due` PMO action-center exception. **D2:** output VAT on progress claims (Cr 2100) and Thai subcontractor WHT (ภ.ง.ด.53, Cr 2361). **D3:** progress-billing certification reconciles with the POC engine — a POC project bills without double-recognising revenue (Cr 1265/2410), a billing project recognises as before. **D4:** the **web workspace** — `/projects/tenders` (estimate→submit→win/lose→award), `/projects/billing` (งวดงาน claim + certify), `/projects/subcontracts` (subcontract + valuation + certify), `/realestate` (developments/units/booking/contract/approve/installments) + nav items (`nav.pm_tenders/billing/subcontracts`) and a new **Real Estate** nav group. Migrations 0249 (release GL, no schema) / 0254 (VAT/WHT columns). Controls PROJ-16/16 amended (RCM stays **186**). `projects` harness 230→**234**; `basics`/`compliance`/`tenant-idx`/`migration-parity`/`rcm-census`/`ts-debt`/`use-client` (244→248) all green; API + web build + typecheck clean. |
...
> **5.5 update (2026-07-08):** the D3-designed **specific-business-tax on transfer** — omitted from the delivered P5 — is now implemented (TAX-09, migration `0285`): per-project `sbt_rate` (NULL = off), atomic **Dr 5840 / Cr 2130** accrual in the transfer JE, `GET /api/tax-reports/pt40` + filing register/remittance calendar (due the 15th). ToE in the `projects` harness. Transfer FEES (government/land-office fees) remain out of scope.
