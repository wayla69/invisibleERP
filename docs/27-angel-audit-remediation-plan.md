...
- **R5-2 (AUD-ARC-09) — DIRECTION SET (docs/28 §4), conversions IN PROGRESS:** server-by-default for new
  pages (`/legal/privacy` shipped as the pattern); top-5 read-heavy conversions (`accounting` ✅ + `eam` ✅ + `projects/[code]` ✅ + `tax/reports` ✅ + `insights` ✅, all 2026-07-02
  — **top-5 complete**; the `use client` CI ratchet (baseline 233) carries the direction forward, with a Gantt client island, `reports`, `insights`) are the rolling work, each
...
| 4.8 | 2026-07-08 | ERP/Product | Debt tranche #5 (R2-5) + unit lane (2.4): as-any 118→98 (shared `rowsOf<T>` driver-shape helper replacing the `.rows`-variance casts; projects null-launders → `!`; lot/sso enum casts → precise types; jsonb/fastify/parity seams untouched); two REAL Thai-text bugs fixed in `text-similarity.ts` found by the new suite (\p{M} marks stripped → 'บริษัท' shredded; NFKC สระอำ decomposition → 'จำกัด' suffix never matched — Thai↔EN company similarity was ~0.2, now 1.0); +25 unit tests (pure-utils, tax-rules: tax-point ม.78/78-1, wht-rates ม.40/ท.ป.4/2528); coverage set +7 modules, thresholds 60/75/62/60 → 78/87/77/78 |
