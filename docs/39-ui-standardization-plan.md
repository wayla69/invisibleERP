...
| 5a+5b (impl) | **5a ModulePage representative slice** — five clean `PageHeader`+`Tabs` top-levels (branches, assets, cost-centers incl. `tabUrlParam`, alerts, claims) become one-liner `<ModulePage title description tabs/>`; `audit` documented as an exception (its filter block is a `Card` form, not a toolbar row). Long tail (~149 files) adopts opportunistically per the signed policy. **5b breadcrumbs** — new shared `components/crumbs.tsx` (no 'use client', next/link via `asChild`); applied to the four nested detail routes: `projects/[code]` (replaces the ad-hoc back-button in the same position), `projects/program/[code]`, `loyalty/members/[id]`, `inventory/[itemId]` (action back-buttons kept — crumbs are additive there). `shop/project/[code]` excluded (requester flow has its own picker nav). 5c toast convention is docs-only, effective immediately. | **DELIVERED** |
...
| 0.9 | 2026-07-09 | Platform / IT | Batch 5a+5b delivered (user go-ahead ต่อเลยได้ไหม = direct sign-off): 5 tabs-mode pages on ModulePage, Crumbs component + 4 nested detail routes; audit-page exception + shop-project exclusion documented. **Track 2.10 batches 0–5 COMPLETE**; steady-state = opportunistic convergence (toast · ModulePage) per the signed policy. |
