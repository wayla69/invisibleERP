...
LINE (prod; `LINE_CHANNEL_ID`) — or accepts a dev id outside production — and mints the **same** member token
for the member whose `line_user_id` is linked (within the tenant); a logged-in member links their LINE via
`POST /api/member/link-line` (one LINE ↔ one member per tenant). The `/m` web app offers both OTP and LINE entry.
