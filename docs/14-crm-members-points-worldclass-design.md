...
Build a **world-class, usable loyalty CRM** for the Oshinei ERP/POS — on par with Thailand's
...
| Member self-service app (LINE LIFF / PWA) | ✅ | **`/m` phone-OTP app — card, points, tier, rewards/redeem, missions/claim, refer, wallet** (shipped) | **LINE LIFF wrapper** |
...
| Analytics: growth, active, **points liability**, redemption | ✅ | `customer_profiles` (RFM), branch KPI | **Liability + redemption funnel** |
...
Extend `compliance/build_rcm.py` → regenerate `Oshinei_ERP_SOX_RCM_v1.xlsx`, and add checks to
...
Member app   POST /api/member/auth/request-otp   phone-OTP login (request)   @Public                 [shipped]
             POST /api/member/auth/verify-otp    verify → mint member token  @Public                 [shipped]
             GET  /api/member/me | /tier | /history | /wallet | /rewards | /missions | /referrals      MemberGuard (self)  [shipped]
             POST /api/member/rewards/:id/redeem | /missions/:id/claim | /refer    self-service       MemberGuard (self)  [shipped]
```
...
services unchanged. LINE LIFF is a thin future wrapper over the same OTP-minted token.
...
| **P4** | Member app & growth | **Referrals + member app + spin-the-wheel + campaign orchestration shipped** (LYL-09; member app `/m`, LYL-10; lucky-draw, MKT-09 / LYL-11; segmented+scheduled campaigns, MKT-10 / LYL-12); partner privileges, churn/redemption analytics staged | Low |
