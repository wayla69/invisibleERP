...
- **Phase 4 — Pricing depth.** Customer-tier & per-branch **price books**; per-line manual-discount approval.
  ...
  - **Phase 4b — manual-discount / bill-discount approval routing (DELIVERED).** A manual line or bill discount
    ...
    **`discount` authorization** (a `pos_overrides` row + new `authorized_pct` column) bounding the % it covers.
    ...
    gate strengthens **R08/R10** + POS-08; census 298). **Phase 4a/4b close the original scope; 4c–4e below add
    pricing + returns depth.**
  - **Phase 4c — scheduled promotions in the UI (DELIVERED).** The price-rule engine already gated a rule on its
    **`valid_from`/`valid_to`** calendar window (alongside the recurring dow/time window) and `POST /api/pricing/rules`
    already accepted them — but the `/pricing → กฎราคา` form never exposed the dates, so a promo couldn't be
    scheduled to run e.g. **1–31 Aug** from the UI. The Rules form now has **เริ่มโปรโมชัน / สิ้นสุดโปรโมชัน**
    (start/end) date fields and the rules table shows a **ช่วงโปรโมชัน** column with a **มีผลตอนนี้ / ยังไม่เริ่ม /
    หมดอายุ** (live / scheduled / expired) badge. Volume/quantity tiers already work via `qty_break` rules
    (best-tier pick), so a 10→5% / 50→10% / 100→15% ladder is three qty_break rules — no engine change. Web +
    harness only (no new control/endpoint/migration). Harness `pricing` 40→43. Manual `01` v0.81; UAT-O2C-576.
  - **Phase 4d — B2B contract pricing (DELIVERED).** A price book can be scoped to a **specific customer**
    (`price_books.customer_code`, migration `0450`) — a negotiated contract price list. The sale supplies
    `customer_code` and only that customer's book (or an unscoped tier/branch book) matches; a matching customer
    contract is **authoritative — it outranks any tier/branch book regardless of priority** (a shop-wide
    ...
    **ลูกค้าเฉพาะราย** field + **ลูกค้า** column on `/pricing → สมุดราคา`. Existing books carry `customer_code`=NULL
    ⇒ no-op tie ⇒ **byte-identical** (golden 588 / writeflow 36 unchanged). Harness `pos-pricebook` 13→16. No new
    numbered control (strengthens **R10**; census 299). Manual `01` v0.82; UAT-O2C-577.
  - **Phase 4e — even / partial exchange at POS (DELIVERED).** `POST /api/pos/exchange` (new **`ExchangeService`**,
    POS bounded context, gated to the refund duty `returns`/`pos_refund`/`exec` — **segregated from selling**,
    ...
    StoreCredit path — restock + GL reversal + auto-issued **ใบลดหนี้** credit note reducing output VAT) and (2)
    rings the **replacement** line(s) paid from that credit first (a new **store-credit tender** on `createSale`
    draws the **2200** deposit liability down before the cash leg — mirrors the dine-in gift-card tender). Only the
    difference moves in cash: **even** swap → zero cash; **up-swap** → cash difference; **down-swap** → residual
    ...
    **byte-identical** (golden 588 / writeflow 36 / restaurant 220 / basics 498 unchanged); no schema change. Web:
    a **แลกเปลี่ยน** button + dialog on `/pos/register` (perm-gated) — pick the bill, set return qty, catalog-add
...
|---|---|---|---|
| 0.16 | 2026-07-20 | Platform | **Phase 4e delivered — even / partial exchange at POS (Phase 4 complete).** `POST /api/pos/exchange` (new `ExchangeService`, POS bounded context, gated `returns`/`pos_refund`/`exec` — SoD R08, segregated from selling) swaps sold item(s) for other(s) in ONE atomic request tx, settled by netting through store credit: RETURN the original line(s) to store credit (existing `createReturn` StoreCredit path — restock + GL reversal + auto-issued ใบลดหนี้ credit note reducing output VAT) and ring the REPLACEMENT paid from that credit first (a new store-credit tender on `createSale` drawing the 2200 deposit liability down before the cash leg — mirrors dine-in gift-card tender). Even swap → zero cash; up-swap → cash difference; down-swap → residual store credit. Both docs commit/roll back together; reason-coded (`EXCHANGE_REASON_REQUIRED`); correlated by an `EXC-` id. Absent the store-credit field a sale is byte-identical (golden 588 / writeflow 36 / restaurant 220 / basics 498 unchanged); no schema change. `store_credit_card_no` on `/api/pos/sales` + `/api/portal/pos/sales`. Web: a **แลกเปลี่ยน** button + dialog on `/pos/register` (perm-gated). Harness `pos-exchange` (15) + phone-viewport `pos-exchange.mobile.spec.ts`; pre-existing `pos-pricebook` + new `pos-exchange` wired into the CI `pos` shard. No new numbered control (strengthens R08/R10; census 299). PN-01 rev 0.39–0.41; manual `01` v0.83/v0.84; UAT-O2C-578; traceability v7.94. (PRs #880/#881/#882.) |
| 0.15 | 2026-07-20 | Platform | **Phase 4d delivered — B2B contract pricing.** A price book can be scoped to a **specific customer** (`price_books.customer_code`, migration `0450`) — a negotiated contract price list. The sale supplies `customer_code`; only that customer's book (or an unscoped tier/branch book) matches, and a matching customer contract is **authoritative — it outranks any tier/branch book regardless of priority** (a shop-wide priority-10 wholesale book cannot undercut a signed contract); among same-scope books the existing precedence holds. `customer_code` on `/api/pos/sales` + `/api/portal/pos/sales` + the `/api/pricing/books` body; web **ลูกค้าเฉพาะราย** field + **ลูกค้า** column on `/pricing → สมุดราคา`. Existing books carry `customer_code`=NULL ⇒ no-op tie ⇒ byte-identical (golden 588 / writeflow 36 unchanged). Harness `pos-pricebook` 13→16. No new numbered control (strengthens R10; census 299). PN-01 rev 0.38; manual `01` v0.82; UAT-O2C-577; traceability v7.93. (PR #879.) |
| 0.14 | 2026-07-20 | Platform | **Phase 4c delivered — scheduled promotions in the UI.** The price-rule engine already gated a rule on its `valid_from`/`valid_to` calendar window and `POST /api/pricing/rules` accepted them, but the `/pricing → กฎราคา` form never exposed the dates. The Rules form now has **เริ่มโปรโมชัน / สิ้นสุดโปรโมชัน** (start/end) date fields + a **ช่วงโปรโมชัน** column with a **มีผลตอนนี้ / ยังไม่เริ่ม / หมดอายุ** (live / scheduled / expired) badge. Volume tiers already work via `qty_break` rules (best-tier pick). Web + harness only (no new control/endpoint/migration). Harness `pricing` 40→43. PN-19 rev 1.58; manual `01` v0.81; UAT-O2C-576; traceability v7.92. (PR #878.) |
