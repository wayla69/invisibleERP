...
| Version | Date | Change |
|---|---|---|
| v0.11 | 2026-07-23 | **§①'s "sentiment tone from TOWS" fact-sheet field DELIVERED via docs/62 Phase 3** (`toneFromTows` — dominant TOWS quadrant → copy tone on the sheet + grounded prompt; no TOWS → neutral), together with per-measurement 95% CI/weak-evidence honesty and the Studio creative A/B (variant B staged on the 50/50 per-member split, measured per arm). Details in docs/62 v0.3 + PN-19 rev 1.75. |
