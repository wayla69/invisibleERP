...
> **Date:** 2026-07-04 · **Status:** v1.4 — **ALL DELIVERED (M0–M4)** + follow-ups (FU1 budget policy, FU2 prefer-stock, FU3 web UI, FU4 raise site cash from the project — all delivered) · **Owner:** ERP / Product
...
|---|---|---|---|
| 1.4 | 2026-07-04 | ERP / Product | **FU4 (follow-up) delivered — raise site cash from the project workspace.** The *เงินสดหน้างาน* tab gains two actions: **ออกเงินทดรอง** (project-tagged advance → `POST /api/finance/advances` with `project_code`/`boq_line_id`) and **ขอเงินสดย่อย** (petty-cash request against a chosen fund → `POST /api/finance/petty-cash/requests`, maker-checker). Both optionally link a **BoQ line** so settlement/approval **consumes** that line's budget (FU1). Web-only over existing endpoints — no API/schema/control/migration change (RCM stays 180). Docs: user-manual 14 rev 2.12; UAT walkthrough `docs/uat/UAT-DOCS32-material-control-walkthrough.md`. `web` build + typecheck green. |
