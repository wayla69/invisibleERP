...
| Raw `CREATE TABLE` DDL | Not journaled → `drizzle-kit migrate` silently skips it in prod. | Drizzle schema (`schema/mmm.ts`) + journaled migration `0405`. |
...
- **This PR (backend + docs):** `schema/mmm.ts`, migration `0405` (+ journal), `modules/mmm` (ingest / model
