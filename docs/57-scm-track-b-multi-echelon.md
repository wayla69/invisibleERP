...
**Status: DRAFT v0.6 · 2026-07-23** · *v0.6: **B4 delivered — Track B COMPLETE.** DRP time-phased roll-up (docs/57 §1.6). New pure `scm-drp.service.ts`: lot-for-lot MRP netting per node (`net_req[t] = max(0, gross_req[t] − sched_receipts[t] − proj_on_hand[t−1])`, lot-sized to the inbound lane moq/pack, planned RECEIPT offset back by the lane lead → planned RELEASE), rolled bottom-up (`drpRollup`) so branch releases become the DC's gross requirements and the DC's releases become the supplier-facing requirements. `ScmNetworkRunService.applyDrp` time-phases every node plan's `orders`; `ScmNetworkPlanService.convertPlan` rolls the DC's supplier-facing releases up to a PR with **one line per planned release** (each with its own required/arrival date) through the existing `ProcurementService.createPr` seam — idempotent by `pr_no`. **No new control, no migration, no engine change, contract unchanged (v2)** — B4 time-phases the quantities the run already computed and reuses the PR writer. ToE `cutover/scm.ts` +6 B4 checks (79/79) — time-phased DC releases, lead-offset + on-hand-netting on the pure primitive, moq/pack lot-sizing, branch→DC sourcing, convert-to-PR. PN-34 §7.21, manual ch.21, UAT §20 (UAT-SCM-074..077). **Track B (B1–B4) fully delivered.*** · *v0.5: **B3 delivered** — DC-shortage allocation fairness (control **SCM-06**). `ScmAllocationService` (`modules/scm-network`) owns the pure rationing primitive `allocateShortage` (proportional / fair_share equal-runout / priority — non-negative, Σ ≤ available, symmetric) + `assertAllocationSound` trust boundary, and the maker-checker'd lifecycle for the **approved allocation policy** (`scm_allocation_policies`, set under new duty `scm_allocate`, approved by a different `scm_approve` holder — new SoD **R25**) and per-plan **overrides** (`scm_allocation_overrides` — refused unless justified `ALLOCATION_OVERRIDE_UNLOGGED`, staged for a second approver, never auto-applied). `ScmNetworkRunService` reads the DC's approved policy and emits fair-share lines on a projected shortage (engine + in-process fallback). Migration **0478** (both tables, canonical 0232-form RLS + tenant-leading index). Permission `scm_allocate` (+ PERM_GROUPS). RCM total 313→314 (impl 310→311). Harness `cutover/scm.ts` +13 SCM-06 checks (73/73). No GL, no golden-master money path, contract unchanged (v2). PN-34 §7.20, manual ch.21, UAT §19 (UAT-SCM-070..073). **B4 (DRP roll-up) remains planned.*** · Owner: Supply-chain / Planning · Depends on **docs/54**
...
| **B4** ✅ | **DRP roll-up.** Time-phased branch→DC→supplier net-requirements (§1.6); supplier-facing releases hand off to the **existing** `ProcurementService.createPr` seam (reuse, do not rebuild). `scm-drp.service.ts` + `applyDrp` + time-phased convert. | L | — |
...
|---|---|---|---|
| 0.6 | 2026-07-23 | Supply-chain / Planning | **B4 delivered — DRP time-phased roll-up; Track B COMPLETE.** New pure `scm-drp.service.ts` (`drpNode`/`drpRollup`/`daysBetweenYmd`): lot-for-lot MRP netting per node — net the period gross requirements against projected on-hand + in-transit scheduled receipts, lot-size to the inbound lane's moq/pack, offset the planned RECEIPT back by the lane lead to get the planned RELEASE (floored at day 0). Rolled bottom-up: branch releases sum into the DC's per-period gross requirements, the DC is netted the same way, and its releases are the supplier-facing requirements (§1.6). `ScmNetworkRunService.applyDrp` time-phases every node plan's `orders` (both engine + fallback paths — orders now carry `{order_ds, arrival_ds, from_node, qty, packs}` per planned release; `orderQty` = Σ). `ScmNetworkPlanService.convertPlan` rolls the DC's supplier-facing releases up to a PR with **one line per release** (each with its own required = arrival date) via the existing `ProcurementService.createPr` seam — idempotent by `pr_no`, falls back to the summed qty for a pre-DRP plan. **No new control, no migration, no engine change, no RCM change, contract unchanged (v2)** — B4 time-phases the quantities the run already computed and reuses the PR writer (no new PR/PO or GL writes). ToE: `cutover/scm.ts` +6 B4 checks (79/79, `scm-mfg` shard) — the DC line carries time-phased releases (order+arrival dates, qty>0), the pure primitive offsets a release back by the lead AND nets demand covered by on-hand to zero releases, moq/pack lot-sizing holds, branch releases are sourced from the DC, and an approved plan converts to a PR. Doc-sync: PN-34 §7.21 (rev 0.14), manual ch.21 (v0.14), UAT §20 (UAT-SCM-074..077) + traceability, docs/55. **Track B (B1 topology → B2 optimization → B3 allocation → B4 DRP) is fully delivered.** |
