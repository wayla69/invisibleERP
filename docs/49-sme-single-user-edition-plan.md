...
|---|---|---|---|
| 1.7 | 2026-07-16 | ERP/Product | **Industry-aware SME nav folding — delivered under docs/50 Track B (B1).** Refines owner decision §6-3 ("every new SME starts identical"): the stamped nav surface is now **per-industry** — `@ierp/shared` `nav-profiles.ts` maps each provisioning industry (restaurant/retail/distribution/services/general) to hidden domains + default-open group keys; `provisionTenant` unions the god defaults with the industry profile into `sme_prefs.{hidden_nav_groups,open_nav_groups,nav_industry}`, `/api/auth/me` surfaces `sme_open_nav_groups`, and the AppShell folds every unlisted subgroup by default (user navFold toggles still win; active route always opens). God per-tenant sme-prefs edits preserve the stamped open profile. First SME login shows ~15 industry items instead of ~210. No new control/migration/permission. Tests: `apps/api/test/nav-profiles.test.ts` (6) + `onboarding` harness (+5). See `docs/50-launch-readiness-roadmap.md`. |
