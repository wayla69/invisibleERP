...
> **Status:** COMPLETE (2026-07-08) — all four god services decomposed per §3/§4: `bi` 1,211→532 (3 PRs),
> `projects` 1,659→1,151 (4 PRs), `procurement` 1,463→979 (4 PRs), `ledger` 1,266→689 (3 PRs) — the
> golden-master 496 paths stayed identical on ALL 14 extraction cuts with zero re-pins (see the §8-log in
> docs/07-backend.md and the revision history below). This was a **refactor of financial-critical services
...
| 1.5 | 2026-07-08 | Platform / IT | `ledger` PR-2 landed — recurring (GL-08) + prepaid (GL-09) → `ledger-recurring.service.ts` (159 LOC; ctor-body, single `postEntry` callback port; cadence helpers moved; templates/schedules verbatim; no parity-locked block touched). Facade 1,107→988 LOC, 7 delegators. Golden identical; basics 293/compliance 138/worldclass 59/multiledger 17/async-jobs 26. Final prescribed ledger cut: PR-3 posting (GL-05 SoD — postEntry/approveEntry/rejectEntry/reverseEntry). |
| 1.6 | 2026-07-08 | Platform / IT | `ledger` PR-3 landed — posting core (GL-05/GL-17) → `ledger-posting.service.ts` (338 LOC; ctor-body, ZERO ports — fully self-contained on db+docNo; postEntry/approve/reject/reverse/immutability/audit + listings verbatim; every internal caller rides the delegator; no parity-locked block touched). Facade 1,266→689 LOC over 3 PRs, 8 delegators. Golden identical; basics 293/compliance 138/worldclass 59/multiledger 17/async-jobs 26/writeflow 36. **ledger COMPLETE per §3 → workstream 2.1 COMPLETE: bi 1,211→532 (3 PRs) · projects 1,659→1,151 (4 PRs) · procurement 1,463→979 (4 PRs) · ledger 1,266→689 (3 PRs) — golden-master 496 paths identical on all 14 extraction cuts, zero re-pins.** |
