...
**Phase 8 (v1.22):** the **Tax** surface — tax invoices (`tax/invoices/page.tsx`), the tax-reports workspace
(`tax/reports/tax-reports-client.tsx`: period picker, Output/Input VAT, P.P.30, filings) and WHT
(`tax/wht/page.tsx`) — is fully `t()`-wired with **136 `tax.*` keys** (professional tax terminology: Output/
Input VAT, WHT, P.N.D./P.P.30, Tax ID, 50 Bis…), reusing `dash.*`/`fin.*`/`inv.*` where shared. Module-level
Thai maps/arrays were refactored to key-based lookup — `THAI_MONTHS`→`tax.month_1..12`, the filing-status
map→a guarded `filingStatusLabel()`, the WHT income-type list→keyed labels, and the doc-type `typeLabel`
moved in-component — and two colliding `t` locals (a `typeLabel` param, an `INCOME_TYPES.map` param) were
renamed. Interpolated toasts/dialogs verified var-for-placeholder. Delegated the mechanical pass to a
subagent, then independently verified (typecheck, build, key-coverage incl. dynamic `tax.month_*`/
`tax.filing_status_*`, interpolation, Thai-leftover=comments-only).

**Phase 9 (v1.23):** the **HR / Payroll** domain — HCM (`hcm/page.tsx`), Payroll (`payroll/page.tsx`: pay-run
prepare/approve/post + statutory remittance) and Employee Self-Service (`ess/page.tsx`: timesheets, leave,
expense claims, payslips) — is fully `t()`-wired with **169 `hr.*` keys** (professional payroll terminology:
pay run, payslip, social security, WHT, leave, OT…), reusing `dash.*`/`fin.*`. The pay-run status map
(`runStatusTh`) was refactored to a key-based `runStatusLabel()` helper, a colliding `t` local (an HCM
tasks-`map` param) was renamed to `tk`, and the pay-run **Reject button** got its own `hr.reject` (imperative
"Reject") distinct from the `fin.rejected` status adjective. Interpolated toasts (timesheet post, pay-run
prepare/approve/reject, remittance, leave submit) verified var-for-placeholder. Delegated the pass to a
subagent, then independently verified (typecheck, build, key-coverage, interpolation, Thai-leftover).

**Phase 10 (v1.24):** the **CRM / Sales** domain — CRM 360 (`crm/page.tsx`), the sales Pipeline
(`pipeline/page.tsx`), Service/tickets (`service/page.tsx`, incl. SLA) and CPQ quotes (`cpq/page.tsx`) — is
fully `t()`-wired with **143 `crm.*` keys** (CRM/sales terminology: opportunity, deal, stage, pipeline, SLA,
quote, subscription, RFM…), reusing `dash.*`/`fin.*`. A colliding `t` local (an SLA-tier `.map()` param in
service) was renamed to `tier`. Interpolated toasts (blast result, deal/contract created, event logged, SLA
breach count) verified var-for-placeholder. Delegated the pass to a subagent, then independently verified
(typecheck, build, key-coverage, interpolation, Thai-leftover=comments-only).

**Phase 11 — completion (v1.25):** the rollout is **finished** — every remaining internal surface is now
`t()`-wired, so the language toggle translates the whole app end-to-end. To keep the catalog reviewable and
parallel-safe, `lib/messages.ts` was split: the inline `BASE_MESSAGES` (core chrome) is merged with ten
per-domain fragments under **`lib/i18n-catalog/`** (`pos`, `loyalty`, `projects`, `financex`, `invops`, `mfg`,
`planbi`, `settings`, `hrx`, `misc`) via `Object.assign` into the exported `MESSAGES`. This phase added the
domains not yet covered — POS/restaurant (`px.*`), loyalty/marketing (`ly.*`), projects/PPM (`pj.*`),
extended finance (`fnx.*`: bank, reconciliation, consolidation, intercompany, deferred tax, FX, leases,
petty-cash, period-close, financial statements/health, GL schedules, revenue, credit-hold, customer/vendor
statements), inventory/warehouse ops (`iv.*`), manufacturing/EAM (`mf.*`), planning/BI (`pb.*`), settings/admin
(`st.*`), HR-extras/pricing (`hx.*`), and the catch-all misc surfaces (`mx.*`) — **~4,000 new keys** total
(th+en). The previously-skipped **shared components** (`data-table` pagination/empty, `state-view` error,
`search-input`, `notification-bell`, `theme-toggle`, `account-statement`, `project-gantt`, `assistant-widget`,
`custom-fields`) and three orphaned pages (`setup`, `costing`, `kds`) were translated too, so no app-wide chrome
is left hardcoded. The two read-only RSC dashboards that can't call the client-only `t()` hook were handled by
extracting client islands (`projects/pipeline` → `pipeline-client.tsx`; `financial-health` moved client-side)
so the server-prefetch architecture is preserved where it matters. Module-level Thai label maps were refactored
to key-based lookups (guarded with raw fallback); colliding local `t` variables were renamed; logic-bearing
emoji prefixes (`✅`/`❌`/`🗑️`), `฿`, machine codes, GL account numbers, enum comparisons, API-payload Thai and
`th-TH` locale strings were deliberately left intact. A whole-app **key-coverage** sweep confirms every
`t('…')` call across `app/(internal)` + `components` resolves (0 missing), and a Unicode Thai-leftover scan
shows only comments/payloads/enums remain. Still pure presentation — no API/route/permission/GL/control change.
...
| 2026-07-04 | v1.25 (IMPLEMENTED) | Web / Product | **i18n rollout Phase 11 — completion** (§1.10): finished the rollout across **all remaining internal surfaces** — POS/restaurant (`px.*`), loyalty/marketing (`ly.*`), projects/PPM (`pj.*`), extended finance (`fnx.*`: bank/recon/consolidation/intercompany/deferred-tax/FX/leases/petty-cash/period-close/financial-statements+health/GL-schedules/revenue/credit-hold/AR-AP statements), inventory/warehouse ops (`iv.*`), manufacturing/EAM (`mf.*`), planning/BI (`pb.*`), settings/admin (`st.*`), HR-extras/pricing (`hx.*`), and misc surfaces (`mx.*`) — **~4,000 new th+en keys**. Split `lib/messages.ts` into `BASE_MESSAGES` + ten `lib/i18n-catalog/*` fragments merged via `Object.assign`. Also translated the app-wide **shared components** (`data-table`/`state-view`/`search-input`/`notification-bell`/`theme-toggle`/`account-statement`/`project-gantt`/`assistant-widget`/`custom-fields`) and three orphaned pages (`setup`/`costing`/`kds`); extracted a client island for the `projects/pipeline` RSC (and moved `financial-health` client-side) to localize without losing server prefetch. Module Thai-maps → key-based helpers (raw fallback); colliding local `t` renamed; `฿`/machine-codes/GL-accounts/enum-comparisons/API-payload Thai/`th-TH` left intact. No API/route/permission/GL/control change. Docs synced (this file). Verified: web **typecheck ✅ + build ✅** + whole-app key-coverage (0 missing) + interpolation-var + Unicode Thai-leftover (comments/payloads only) checks. |
| 2026-07-04 | v1.24 (IMPLEMENTED) | Web / Product | **i18n rollout Phase 10** (§1.10): `t()`-wired the **CRM / Sales** domain — CRM 360, sales Pipeline, Service/SLA and CPQ quotes — **143 `crm.*` keys** reusing `dash.*`/`fin.*`; renamed a colliding `t` local (`tier`). No API/permission/GL/control change. Docs synced (this file). Verified: web typecheck ✅ + build ✅ + key-coverage + interpolation-var + Thai-leftover checks. |
| 2026-07-04 | v1.23 (IMPLEMENTED) | Web / Product | **i18n rollout Phase 9** (§1.10): `t()`-wired the **HR / Payroll** domain — HCM, Payroll (pay-run + remittance) and ESS (timesheets/leave/claims/payslips) — **169 `hr.*` keys** reusing `dash.*`/`fin.*`; refactored the pay-run status map to a helper, renamed a colliding `t` local (`tk`), and added `hr.reject` (imperative) for the pay-run reject button. No API/permission/GL/control change. Docs synced (this file). Verified: web typecheck ✅ + build ✅ + key-coverage + interpolation-var + Thai-leftover checks. |
| 2026-07-04 | v1.22 (IMPLEMENTED) | Web / Product | **i18n rollout Phase 8** (§1.10): `t()`-wired the **Tax** surface — tax invoices, tax-reports workspace (Output/Input VAT, P.P.30, filings) and WHT — **136 `tax.*` keys**, reusing `dash.*`/`fin.*`/`inv.*`; refactored `THAI_MONTHS`/filing-status/WHT-income-type module maps to key-based helpers and renamed two colliding `t` locals. No API/permission/GL/control change. Docs synced (this file). Verified: web typecheck ✅ + build ✅ + key-coverage (incl. dynamic month/status keys) + interpolation-var + Thai-leftover checks. |
