...
| UAT-O2C-286 | Cash application — one receipt across many invoices, remainder parks on-account (GL 2220) | REV-20 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-287 | Apply-on-account later clears 2220 into 1100; `INSUFFICIENT_UNAPPLIED` when exhausted | REV-20 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-288 | Over-applying an invoice rejected (`OVER_APPLIED`, pending applications count) | REV-20 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-289 | Allocations beyond the receipt rejected (`APPLY_EXCEEDS_RECEIPT`) | REV-20 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-290 | Cross-customer application rejected (`CUSTOMER_MISMATCH`) | REV-20 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-291 | ≥ THB 100k application parks PendingApproval; self-approve `SOD_VIOLATION`; distinct approver applies | REV-20 (mirrors REV-16) | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-292 | Reversal reason-required + audited; cash returns on-account; `ALREADY_REVERSED` on repeat | REV-20 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-293 | Issued AR-linked credit note applies as a credit line (`CN_*` guards; statement shows it) | REV-20, TAX-07 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
