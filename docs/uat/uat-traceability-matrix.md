...
| UAT-O2C-284 | LINE e-receipt to the sale's member — flex card + opaque public full-receipt link (POS-2) | REST-10 unchanged (delivery-channel feature — no new control) | 20 §7 (11), rev 3.12 (`receipts.ts`) |
| UAT-O2C-285 | LINE e-receipt negatives — `LINE_NOT_LINKED` / forged link `BAD_TOKEN` / prod `LINE_NOT_CONFIGURED` | REST-10 unchanged (delivery-channel feature — no new control) | 20 §7 (11), rev 3.12 (`receipts.ts`) |
