...
| UAT-INV-106 | Create a two-step transfer order (Draft, document only); `from==to` → `SAME_LOCATION` | INV-16 | 03 §7 (15), rev 1.4 (`transfer-orders.ts`) |
| UAT-INV-107 | Ship → relieve source + `Dr 1255 / Cr 1200`; re-ship → `NOT_DRAFT` | INV-16 | 03 §7 (15), rev 1.4 (`transfer-orders.ts`) |
| UAT-INV-108 | In-transit aging / period-end cutoff report (days-in-transit + buckets) | INV-16 | 03 §7 (15), rev 1.4 (`transfer-orders.ts`) |
| UAT-INV-109 | Custody SoD — shipper cannot receive own transfer (`SOD_SELF_APPROVAL`) | INV-16 · R11 | 03 §7 (15), rev 1.4 (`transfer-orders.ts`) |
| UAT-INV-110 | Receive (≠ shipper) → land dest + `Dr 1200 / Cr 1255`; sub-ledger reconciles to GL 1200; `NOT_SHIPPED`/`NEG_STOCK` | INV-16 | 03 §7 (15), rev 1.4 (`transfer-orders.ts`) |
