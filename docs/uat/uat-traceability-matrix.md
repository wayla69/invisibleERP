...
| UAT-GL-171 | CLS-01 flux generate flags the lines breaching abs+pct thresholds (revenue/marketing breach; COGS below abs does not) | GL-25 | 05 rev 0.41 (`flux-analysis.ts`) |
| UAT-GL-172 | CLS-01 sign-off refused while a breached line is unexplained (UNEXPLAINED_LINES) | GL-25 | 05 rev 0.41 (`flux-analysis.ts`) |
| UAT-GL-173 | CLS-01 a non-breached line cannot be explained (LINE_NOT_BREACHED) | GL-25 | 05 rev 0.41 (`flux-analysis.ts`) |
| UAT-GL-174 | CLS-01 forced explanation → maker-checker sign-off (self-review SOD_SELF_APPROVAL; distinct reviewer certifies; ALREADY_CERTIFIED locks) | GL-25, R05 | 05 rev 0.41 (`flux-analysis.ts`) |
| UAT-GL-175 | CLS-01 prior_year/budget comparatives + BS-basis filter (BUDGET_PL_ONLY) + RLS isolation | GL-25, ITGC-AC-03 | 05 rev 0.41 (`flux-analysis.ts`) |
