...
| UAT-O2C-296 | Cash application — one receipt across many invoices, remainder parks on-account (GL 2220) | REV-21 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-297 | Apply-on-account later clears 2220 into 1100; `INSUFFICIENT_UNAPPLIED` when exhausted | REV-21 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-298 | Over-applying an invoice rejected (`OVER_APPLIED`, pending applications count) | REV-21 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-299 | Allocations beyond the receipt rejected (`APPLY_EXCEEDS_RECEIPT`) | REV-21 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-300 | Cross-customer application rejected (`CUSTOMER_MISMATCH`) | REV-21 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-301 | ≥ THB 100k application parks PendingApproval; self-approve `SOD_VIOLATION`; distinct approver applies | REV-21 (mirrors REV-16) | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-302 | Reversal reason-required + audited; cash returns on-account; `ALREADY_REVERSED` on repeat | REV-21 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
| UAT-O2C-303 | Issued AR-linked credit note applies as a credit line (`CN_*` guards; statement shows it) | REV-21, TAX-07 | 01 §7 (8e), rev 0.30 (`basics.ts`) |
