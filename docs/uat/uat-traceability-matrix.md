...
| UAT-P2P-141 | Sensitive master-data edit stages a request — master unchanged while pending; self-approval → `SOD_SELF_APPROVAL` | MDM-01 (new), R22 | 02 §7 (1), §9, §13 (`masterdata-change.ts`) |
| UAT-P2P-142 | A distinct approver applies the staged change to the master; encrypted bank account round-trips | MDM-01 (new) | 02 §7 (1), §9 (`masterdata-change.ts`) |
| UAT-P2P-143 | Reject discards the change (master untouched); `payment_terms` changes only via the maker-checker (off the direct edit) | MDM-01 (new) | 02 §7 (1), §9, §13 (`masterdata-change.ts`) |
| UAT-P2P-144 | Non-sensitive field edits directly; non-sensitive field refused (`FIELD_NOT_SENSITIVE`); RLS tenant isolation (`NOT_PENDING`) | MDM-01 (new) | 02 §7 (1), §13 (`masterdata-change.ts`) |
