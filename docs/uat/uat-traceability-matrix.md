...
| UAT-GL-171 | CLS-02 open a disclosure checklist seeds the standard TFRS/SEC items (Draft; idempotent per period) | GL-26 | 05 rev 0.41 (`disclosure-checklist.ts`) |
| UAT-GL-172 | CLS-02 review blocked while any item is Open → ITEMS_INCOMPLETE | GL-26 | 05 rev 0.41 (`disclosure-checklist.ts`) |
| UAT-GL-173 | CLS-02 maker-checker: preparer self-review → SOD_SELF_APPROVAL; distinct reviewer reviews→issues; Issued immutable (NOT_DRAFT) | GL-26, R05 | 05 rev 0.41 (`disclosure-checklist.ts`) |
| UAT-GL-174 | CLS-02 DISC support evidence pins to doc_attachments (assertDocExists) + tenant RLS isolation | GL-26, ITGC-AC-03 | 05 rev 0.41 (`disclosure-checklist.ts`) |
