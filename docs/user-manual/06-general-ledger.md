...
- **Issuance review & approval (maker-checker, GL-29).** Before the statements are relied upon, govern their
  issuance: the **การสอบทานและอนุมัติงบการเงิน** panel on **งบการเงิน → งบตามกฎหมาย** lets a preparer **ส่งอนุมัติ**
  a fiscal year's statements — the system snapshots the key figures (assets/liabilities/equity/revenue/net income)
  with a hash — and a **different** user **อนุมัติ** them (approving your own → `SOD_VIOLATION`). Once approved, the
  formatted FS pack stamps **"Reviewed & approved by ⟨name⟩ on ⟨date⟩"** instead of *"Unaudited"*; if the GL
  figures change after approval the stamp becomes **"figures changed since approval — re-review required"**, so a
  stale approval can't masquerade as current. The pending review also appears in the **รายการรออนุมัติ**
  (`/approvals`) center. `POST /api/reports/fs/statement-pack/submit`, `POST …/statement-reviews/:id/approve`,
