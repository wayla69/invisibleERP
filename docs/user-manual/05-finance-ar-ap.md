...
1. On the **AR** table, find the invoice and click **Receive payment**
   (**รับชำระ**) — or open the dialog from the header button and pick the invoice from
   the **dropdown of invoices with an outstanding balance** (each shows customer +
   outstanding; picking one pre-fills the amount). Choose **พิมพ์เลขเอกสารเอง…** to key an
   invoice number manually.
2. Confirm the **amount**, **method** (e.g. Transfer) and a **reference number**.
3. Save.
...
### To import a bank statement from a file (นำเข้าไฟล์จากธนาคาร)

On the **Bank** screen (`/bank`), open an account's reconciliation panel and click
**นำเข้าไฟล์ Statement (CSV/XLSX)**:

1. In your bank's internet banking (KBank / SCB / BBL / others), export the account
   statement as **CSV or Excel** for the period.
2. Upload the file as-is — the system finds the columns from the bank's own Thai or
   English headers (วันที่/Date, รายการ, ถอน/ฝาก or a signed Amount, คงเหลือ/Balance),
   converts Buddhist-Era dates automatically, and derives the opening/closing balance
   from the running-balance column.
3. The import runs **auto-match** immediately and the toast reports how many lines
   were imported and matched. Summary/footer rows in the file are skipped and counted.

**If the file is rejected** (`NO_DATE_COLUMN` / `NO_AMOUNT_COLUMN` / `BAD_FILE`): the
export doesn't have a recognizable date or amount column — re-export choosing the
detailed/transaction format, or paste the statement into the CSV template. Nothing is
partially imported on a rejected file.
