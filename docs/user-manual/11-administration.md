...
- **SME — คนเดียวทำได้ทุกงาน** — for a company run by a single operator. The operator may approve their
  **own** items (journal entries, AP payments, petty cash, master-data changes, quotes, stocktakes,
  timesheets/leave, price rules, gift cards, reconciliation certifications) **only** by supplying a
  justification each time (without one the API answers `400 SELF_APPROVAL_REASON_REQUIRED`; the web
  prompts for the reason automatically). Every self-approval is recorded in the `self_approvals`
