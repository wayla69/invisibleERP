...
pick the PO from the **dropdown** (the same list as the PO table above — picking it loads its
attachments immediately; choose **พิมพ์เลขเอกสารเอง…** for an older PO), and click **แนบรูป/ไฟล์** (photo or PDF, max ~2MB). Anyone who handles the
...
2. Pick the supplier invoice from the **dropdown of outstanding AP bills**, and (optionally)
   the PO from the dropdown of open POs — no typing of document numbers. Choose
   **พิมพ์เลขเอกสารเอง…** at the bottom of either list to key an unusual number (e.g. a
   procurement-only user, whose role cannot read the AP list, falls back to manual entry).
   The **find** box on the right offers already-run matches the same way.
...
4. If the document had no usable PO reference, the screen shows **scored PO
   candidates** (by vendor + amount). Click one, or pick any PO from the full open-PO
...
2. **Submit an invoice** — on the **ใบแจ้งหนี้** tab the vendor enters the invoice
   number, amount and VAT (optionally picking one of **their own POs** from a dropdown —
   no PO number to remember) and submits it. This creates
