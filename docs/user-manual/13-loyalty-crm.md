...
- **เพดานรวมทุกช่องทาง:** จำกัดจำนวนข้อความการตลาดต่อสมาชิกต่อ 7 วัน — นับรวม LINE/SMS/อีเมลจากทุกเครื่องมือ
  รวมถึง**แคมเปญบรอดแคสต์** (สมาชิกที่ถึงเพดานแล้วจะถูกข้ามในแคมเปญด้วย) ส่วนที่เกินถูกข้ามพร้อมบันทึก `global cap`
...
2. **What the member sees** — a digital **card** (name, code, tier, points balance) with a **tier-ladder
   ...
   *Loyalty settings* (`เพดานโอนแต้มต่อวัน`; `0` switches the feature off), can't send more than their
   ...
   > **Control (large staff transfers need a second person).** A **staff-initiated** transfer of **more than 500 points**
   > is **held for approval** and moves **no points** until a **different** authorised user (**Approvals/Exec**) approves it
   > (`POST /api/loyalty/transfers/:reqNo/approve`; the pending queue is `GET /api/loyalty/transfers/pending`, and
...
(`/crm/members`) pre-filtered, where you can fire a targeted campaign at exactly that group. Segments are computed from
