...
| Customize how documents look (receipt · **quotation · purchase order · payslip · full tax invoice (ใบกำกับภาษีเต็มรูป) · abbreviated tax invoice (ใบกำกับภาษีอย่างย่อ, 80mm slip)**) — accent colour, logo, header note, show/hide seller address·tax-id, amount-in-words, signature captions, footer terms (the slip honours only its header/footer notes). Tax-document mandatory fields always print (§86/4, §86/6). | **Document templates** | `/document-templates` | users / exec | [Administration §14](./11-administration.md) |
