...
### To do an exchange (even / partial)

When a customer swaps an item for a different one, you don't need to run a
separate refund and a new sale — do it in **one step** from the register.

**Required permission:** the same as a return (`returns` / `pos_refund` /
`exec`) — a refund is involved, so a plain `pos_sell` cashier doesn't see the
button.

1. On **`/pos/register`**, tap **แลกเปลี่ยน** (top of the register).
2. Pick the **original bill** (บิลเดิม) from the list, or type the sale number.
3. Under **สินค้าที่คืน**, set how many of each line the customer is **returning**.
4. Under **สินค้าที่รับแทน**, search and add the **replacement** item(s) (the
   price fills in from the catalogue; adjust the quantity or price if needed).
5. A live preview shows the **returned value vs the replacement value** and
   whether the customer **pays the difference** or is **left with store credit**.
6. Enter a **reason** (required) and tap **ยืนยันการแลกเปลี่ยน**.

**Expected result:** one all-or-nothing transaction — the old item is returned
to **store credit** and the replacement is rung and paid from that credit, so
the customer settles **only the difference**:

- **Even swap** (same value) → **no money** changes hands.
- **More-expensive** item → the customer **pays the difference** in cash.
- **Cheaper** item → the balance **stays as store credit** the customer keeps.

If the returned item had a full tax invoice, the system auto-issues the matching
**ใบลดหนี้ (credit note)** so VAT is corrected. If either half can't complete,
**nothing changes** (the whole exchange rolls back). The result panel shows the
**exchange number (`EXC-…`)**, the new sale number, and the cash collected or the
remaining store credit.

[screenshot: exchange dialog — return lines, replacement search, net preview]
