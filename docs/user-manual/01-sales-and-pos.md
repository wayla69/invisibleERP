...
2. The guest opens the **เมนู** tab, browses by category, picks options
   ...
   **filter bar** (ทั้งหมด / ⭐ แนะนำ / each category) narrows the list, and any
   dish you flagged **เมนูแนะนำ** shows first in a highlighted **Recommended** row.
...
**Clear a finished ticket — scan or tap.** When a runner delivers an order, either
**scan the order number** into the box at the top of the Board, or tap
**เสิร์ฟทั้งออเดอร์** on the ticket (Board-by-table or the Expo card): every
**พร้อมเสิร์ฟ** line on that order is marked served in one go, so no card is left
