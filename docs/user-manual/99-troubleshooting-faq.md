...
| A workflow item stays **pending forever** in an SME (single-operator) company | The approval workflow step was configured with **`all_of_n` > 1** — it needs N *different* people to approve, and a one-person company can never supply the second approver. SME self-approval covers the maker-checker rule, not the N-distinct-approvers count. | Edit the workflow definition (`/workflow`) and set the step's `all_of_n` to 1 (or remove the multi-approver step). |
