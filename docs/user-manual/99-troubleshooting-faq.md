...
| `SCOPE_EXCEEDS_GRANT` | You tried to issue an **API key** carrying a permission/scope your own account doesn't hold. | An API key can only be granted scopes the person creating it already has — so a narrow access-admin can't mint a broadly-scoped key and escalate. Remove the listed scope(s), or ask someone who holds that permission to issue the key. See [Administration](./11-administration.md). |
