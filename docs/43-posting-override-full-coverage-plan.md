...
| Date | Version | Change |
|---|---|---|
| 2026-07-11 | 0.11 | **PR-9 (UI overhaul) DELIVERED — SERIES COMPLETE (9/9).** `/setup/posting-rules` becomes a four-tab registry workspace inside the existing `'use client'` island (ratchet flat): **ทะเบียนเหตุการณ์** — per-event cards over `GET …/registry` (role/side/REAL default/tier badge + the tenant's current override with GL-24 status; search + tier filter; summary stats; ตั้งค่า shortcut pre-fills the editor for `free` roles), **ตั้งค่า & ทดลอง** — the PR-1 editor + preview, account field now a postable-COA datalist picker (fail-soft; server stays fail-closed) + **ปิดใช้** on approved rules, **คิวรออนุมัติ** — every pending override across all events with approve/reject, **ประวัติการแก้ไข** — `GET …/audit` newest-first. Web-only: no control/API/schema change (RCM untouched; golden N/A); `compliance` 173 + `basics` 407 re-run green; web build + both ratchets green. No mobile spec added (desktop back-office workspace; web-e2e suite is the gate). UAT-GL-174; PN-04 rev 2.25; manual 06 v0.13; traceability v6.75. **All nine docs/43 PRs are now landed — posting-override full coverage is DELIVERED.** |
| 2026-07-11 | 0.10 | **PR-8 (reporting completeness + bulk IO) DELIVERED**: migration `0346` — accounts.cf_bucket/cf_label/is_current; SCF classifies column → CF_CLASSIFY → type fallback (a new account never lands unclassified once declared); metrics honour is_current (column → lists → numbering); /chart-of-accounts dialog captures both. Bulk IO (Q4): registry `accounts` (COA_ADMIN_ONLY + sensitive→staged, Type pgEnum-normalized) and `posting_rules` (every row via upsertRule → GL-24 PendingApproval; per-row errors, fail-closed without the service). ToE basics 407 + ext 299 (4 checks); sweep + golden unchanged. UAT-GL-173. |
| 2026-07-11 | 0.9 | **PR-7 (tie-out widening) DELIVERED**: `LedgerService.postingAccountSet` = {registry default} ∪ {approved override}; the PAY-02 schedule + remit (payroll.service — schedule lines expose `account_set`; remit accepts any set member and clears that SPECIFIC account), the PND3/53 tie-out and the ภ.ธ.40 report (tax-reports.service — responses gain `gl_account_set`) read the widened sets. Roles flipped Tier B→A: PAYROLL.SSO.sso_payable / PAYROLL.WHT.wht_payable / PAYROLL.PF.pf_payable / APPAY.WHT.wht_payable / SBT.TAX.sbt_payable. NOT widened: PP30/PP36 VAT sets (2100/1300/2120), 2410 progress ties (stay Tier B), REC-04 five (pinned permanently, Q3). ToE payroll 46 (4 checks); sweep compliance/taxdocs/input-vat/basics/worldclass + golden unchanged. UAT-GL-172. |
