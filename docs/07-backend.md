...
| 2026-07-13 | `billing` | docs/46 Phase 4c: BillingService decomposed — 4 ctor-BODY cuts. `stripe-gateway.ts` (ONE StripeBilling instance replaces 3 ad-hoc news); `tenant-provisioning.service.ts` (ITGC-AC-18 gate + invites + approval queue + provisionTenant w/ L-4 refusal + factoryResetTenant verbatim + resolveTenantId; `isSignupAllowed`/`SignupDto` re-exported); `platform-admin.service.ts` (cross-company console ops); `billing-metering.service.ts` (AI + e-Tax/POS meters, idempotent overage runs — the two engines deliberately NOT unified yet). | Facade 1,094→**369 LOC** (plans, subscription/changePlan, checkUserLimit, checkout + webhook state machine, delegators); ctor untouched; off the size-ratchet list (11→10). Gated: onboarding 86 · sub-billing 6 · pg-core 15 · bi 44 · async-jobs 26 · golden 518 identical. |
| 2026-07-13 | `line-webhook` | docs/46 Phase 4d: the 19 stateless chat command actions → `line-chat-actions.service.ts`; the 6 ModuleRef lazy lookups (circular module graph) become an explicit `ChatActionPorts` interface handed down from the webhook. Webhook keeps matcher/dispatch (precedence IS behaviour), throttle/dedupe/reply plumbing, and the stateful attach/capture/postback/copilot flows. Per-module handler registry consciously deferred (docs/46 rev 1.0). | Controller file 978→**593 LOC**, off the size-ratchet list (10→9). Gated: line-crm 143 · restaurant 186 · unit 472; ratchets green. |
| 2026-07-13 | `ledger` | docs/46 Phase 4e (facade round 2 — Phase 4 COMPLETE): 3 ctor-BODY cuts. Cut 1 `ledger-reporting.service.ts` (trialBalance snapshot+FIN-7a dimension paths, accountLedger, listDimensions, incomeStatement + byBranch, balanceSheet, perAccountNet, gaapComparison, the canonical `aggregateByType` + `ledgerCond` — both public: they feed the cashflow ports and closeYear; `assertLedger` stays a facade port; `DimensionFilter`/`round4` re-exported). Cut 2 `ledger-periods.service.ts` (per-tenant fiscal calendar, closePeriod/openPeriod/provisionFiscalYear, GL-05-drafted postOpeningBalances, closeYear P&L sweep, TFRS-15 accrueLiability — posting rides the `postEntry` port so GL-05/GL-21 + period gates stay in LedgerPostingService; exports `resolveTenantId`). Cut 3 GL-24 override resolution (`resolvePostingOverrides`/`resolvePostingAccountSet`/`resolvePostingOverridesMany`) → `posting-overrides-cache.ts` beside its TtlCache (NOT posting.service.ts — that import direction is a require cycle with PostingService's LedgerService injection; deviation recorded in docs/46 rev 1.1). | Facade 926→**304 LOC** (keeps CoA seed/provision/curation, multi-GAAP masters, postAdjustment, alreadyPosted, ~40 delegators); ctor `(db, docNo)` untouched (goldenmaster/writeflow positional). Off the size-ratchet list (9→8). Gated: basics 414 · compliance 179 · worldclass 59 · multiledger 17 · writeflow 36 · golden 518 identical (no re-pin) · unit+coverage 472; all four ratchets green. |
| 2026-07-13 | (all) | docs/46 Phase 5 (hygiene — plan COMPLETE): (1) the four single-file `.module.ts` god files (customers · crm/accounts · crm/account-depth · crm/account-health) split verbatim into conventional `service/controller/module` files, zod bodies exported from the services, classes re-exported from the module files (import sites untouched); (2) `posting-events.ts` composed from nine per-domain definition files + `posting-events.types.ts` — 94/94 events deep-compared identical, `postingDefault()`/`postingRole()`/`assertPostingEventDefaults` unchanged as the single API; (3) `app.module.ts` flat 140-entry import array → ten pure-wiring domain aggregates in `src/domains/` (each re-exports its members so the APP_GUARD chain keeps resolving in AppModule's injector). | New feature modules land as a one-line change in the owning `src/domains/*` file; new posting events land in the owning `posting-events.<domain>.ts`. Gated: basics 414 · compliance 179 · golden 518 identical · worldclass 59 · multiledger 17 · writeflow 36 · restaurant 186 · onboarding 113 · pipeline 132 · crm 56 · pdpa 40 · bi 44 · ext 299 · unit 472; all five gate scripts green. |
