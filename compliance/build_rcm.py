...
add("GOV-01","Monitoring","Application","All","A maker-checker approval sits un-actioned and is forgotten — a transaction stalls before it is effective, or (worse) a control is quietly bypassed and no one notices an item never got its independent review.","Authorization/Completeness",
    "Pending-approvals MONITOR (COSO monitoring): a single worklist of EVERY item awaiting independent approval across the system, each with its AGE in days and an overdue roll-up — spanning the manual-JE (GL-05), bank-adjustment (BANK-02), AP-disbursement (EXP-06), payroll (PAY-03), asset-revaluation (FA-08), asset-disposal (FA-09), inventory-write-off (INV-07), FX-rate (FX-04) and budget (BUD-01) maker-checkers. The controller reviews it before close; a stale item (age beyond the threshold) is a control finding to chase or escalate. Read-only; tenant-scoped via the caller's RLS.","Det","Automated","Continuous / pre-close","Controller / CFO","P16",
    "finance.service.ts pendingApprovals; finance.controller.ts GET approvals/pending (exec/approvals/creditors); aggregates journalEntries(Draft incl. BANKADJ→BANK-02)/apPayments/payruns/asset_revaluations/fixed_assets(disposal_pending)/inv_writeoff_requests/fx_rates(PendingApproval)/budgets(PendingApproval); cutover/compliance.ts (ToE)","Inspect the aggregation across all pending sources + the age/overdue roll-up.","Re-perform: create a pending item (e.g. a Draft JE) and confirm it appears in the worklist with its control ID, amount and age; the overdue count flags items past the threshold (re-performed by the harness).","Pending-approvals worklist","Implemented")
