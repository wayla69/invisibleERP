...
add("REV-12","Revenue & Cash","Application","Accounts Receivable","Overdue receivables not pursued; collection lapses; further credit extended to a defaulting customer.","Valuation/Authorization",
    ...
    "collections.service.ts (worklist, recordDunning, creditStatus, creditCheck, SERIOUS_OVERDUE_DAYS); pos.service.ts (createOrder CREDIT_OVERDUE); collections.controller.ts; cutover/basics.ts (ToE)","Inspect aging→stage logic, hold decision + order-entry gate.","Worklist ages open AR; dunning recorded; held customer's credit-check denied; a 90+ defaulter is blocked at order entry while a good-standing customer orders (re-performed by the harness).","Dunning log; credit-hold report; CREDIT_OVERDUE rejections","Implemented")
