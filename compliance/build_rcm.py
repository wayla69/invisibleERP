...
# ---- Monitoring ----
add("MON-01","Monitoring","Application","All maker-checker accounts","A maker-checker is defeated in practice if the checker never acts — a transaction sits unapproved for weeks, tying up cash and leaving the books mis-stated, with no one watching the queue. Stale pending approvals are a silent control breakdown.","Authorization/Completeness",
    "Pending-approvals aging monitor (GET /api/finance/approvals/aging): ONE read across every maker-checker queue, aged by how long each item has waited for its second signature. Scans every Draft journal entry — so it AUTO-COVERS any control that posts a Draft JE via ledger.postEntry pendingApproval (GL-05 manual JE, PAY-03 payroll, FA-08 revaluation, FA-09 disposal, BANK-02 bank adjustment) — PLUS the queues that post nothing until approval (INV-07 inventory write-offs, AP-PAY vendor payments, FX-04 manual FX rates, BUD-01 budgets). Each item is control-tagged, attributed to its requester, valued, and aged; items past the SLA (default 7 days, ?stale_days overrides) are surfaced as exceptions with an all_clear flag and per-control roll-up. The supervisory 'is anything stuck in approval?' view over the whole control environment.","Det","Automated","Daily / on demand","Controller / CFO","P16",
    "finance.service.ts pendingApprovalsAging; finance.controller.ts GET approvals/aging (exec/approvals/gl_close); scans journal_entries Draft + inv_writeoff_requests + ap_payments; cutover/compliance.ts (ToE)","Inspect the monitor: the Draft-JE scan + the two non-JE queues + the aging buckets + the SLA/exceptions roll-up.","Re-perform: post a Draft maker-checker item, confirm it surfaces in the monitor aged & control-tagged and is NOT stale when fresh; tighten the SLA and confirm it is flagged as an exception (all_clear=false). Re-performed by the harness.","Aging report; exceptions list","Implemented")
