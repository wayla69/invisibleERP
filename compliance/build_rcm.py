...
add("EXP-05","Expenditure","Application","Operating Expense; Accounts Payable","Employee/maintenance spend recorded in GL but not as a payable (escapes AP aging; AP sub-ledger ≠ GL control).","Completeness/Accuracy",
    "ESS expense approval and EAM work-order completion raise an AP payable via createApTxn (Dr 5100/5700 / Cr 2000) — not a bare GL post — so the liability appears in AP aging, settles through the AP pay flow, and keeps the AP sub-ledger ↔ GL 2000 reconciled (REC-01). Reimbursements vat-exempt; SoD approver ≠ claimant retained.","Prev","Automated","Per claim/WO","AP / Controller","P10",
    "ess.service.ts (approveExpense → createApTxn); eam.service.ts (updateWorkOrderStatus → createApTxn, acct 5700); finance.service.ts (createApTxn expense_account/tenant_id); cutover/ess.ts + basics.ts (ToE)","Inspect approval→AP routing + reconciliation.","Approve a claim / complete a WO → AP payable raised, paid via AP, sub-ledger ties to GL 2000 (re-performed by the harness).","AP sub-ledger; reimbursement/maintenance payables","Implemented")
