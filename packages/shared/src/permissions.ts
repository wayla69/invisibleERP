...
export const PERMISSIONS = [
  ...
  'cpq', 'cpq_approve',
  // ── Quality / CAPA single-duty split (QMS-2, QC-02; SoD R21) — owning/driving a corrective-and-preventive
  //    action (root-cause, action plan, submit for verification) is segregated from the INDEPENDENT
  //    effectiveness verification that closes it. `quality` is the CAPA-owner duty (create/own CAPAs + child
  //    actions, submit for verification); `quality_approve` is the independent verifier duty (verify → close
  //    effective, or reopen ineffective). Standalone granular perms: NOT implied by a coarse perm. Endpoints
  //    gate `quality*/exec` so existing exec roles keep working; the in-app self-approval block
  //    (verifier ≠ owner/created_by) is the real control (QC-02) regardless of the permission held. ──
...
export const SOD_RULES: SodRule[] = [
  ...
  { id: 'R21', dutyA: 'Own / drive a CAPA', dutyB: 'Verify CAPA effectiveness & close',
    a: ['quality'], b: ['quality_approve'], severity: 'High', risk: 'Own a corrective/preventive action and sign off its own effectiveness — close a CAPA whose root cause was never actually fixed, defeating the quality corrective-action loop.', mitigation: 'Separate CAPA ownership from effectiveness verification; maker-checker enforced in-app (verified_by ≠ owner/created_by → SOD_SELF_APPROVAL, QC-02).' },
];
