...
import { sql, eq, and, desc, gt, lte, inArray } from 'drizzle-orm';
import type { JwtUser } from '../../common/decorators';
import { DRIZZLE, type DrizzleDb } from '../../database/database.module';
import { accounts, journalEntries, fiscalPeriods, ledgers, posMembers, posMemberLedger, loyaltyConfig, loyaltyPostingRuns, tenantAccounts, postingRules } from '../../database/schema';
...
import { LedgerReportingService, round4, type DimensionFilter } from './ledger-reporting.service';
...
export class LedgerService implements OnModuleInit {
  ...
  async closeYear(fiscalYear: number, createdBy: string, ledgerCode: string = LEADING, tenantId?: number | null) {
    ...
    const rows = await this.reporting.aggregateByType(db, from, to, undefined, ledgerCode, tid);
    const lines: JournalLineDto[] = [];
