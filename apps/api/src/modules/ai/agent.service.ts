...
const SYSTEM_CACHED = [{ type: 'text', text: SYSTEM_TH, cache_control: { type: 'ephemeral' } }];
...
export class AgentService {
  ...
  private selectModel(messages: any[]): string {
    ...
    if (Array.isArray(last?.content) && (last.content as any[]).some((b: any) => b.type === 'tool_result')) {
      return 'claude-haiku-4-5-20251001';
    }
    return 'claude-sonnet-4-6';
  }

  // ITGC-SEC-AI-01 — per-tenant daily token budget.
  // Reads the plan's ai_tokens_daily feature (-1 = unlimited) and today's cumulative usage.
  // Fail-open on DB errors (tracking outage must not block the AI feature).
  private async checkBudget(tenantId: number | null): Promise<void> {
    if (tenantId == null || !this.sql) return; // HQ Admin has no limit; skip if sql unavailable
    try {
      const [plan] = await this.sql<{ limit: number }[]>`
        ...
        LIMIT 1`;
      const limit = plan ? Number(plan.limit) : 50000;
  ...
  private async recordUsage(tenantId: number | null, input: number, output: number): Promise<void> {
    if (tenantId == null || (!input && !output) || !this.sql) return;
    try {
      await this.sql`
        INSERT INTO ai_token_usage (tenant_id, usage_date, input_tokens, output_tokens)
  ...
  async chat(message: string, history: any[] = [], _user: JwtUser): Promise<{ reply: string; history: any[] }> {
    ...
    await this.checkBudget(_user?.tenantId ?? null);
    ...
    try {
      ...
      await this.recordUsage(_user?.tenantId ?? null, totalInput, totalOutput);
    }
