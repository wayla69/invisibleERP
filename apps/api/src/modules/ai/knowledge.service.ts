...
export class KnowledgeService {
  ...
  async ingest(dto: { title: string; source?: string; content: string }, user: JwtUser) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      rows.push({ tenantId: user.tenantId ?? null, docId: Number(doc.id), ord: i, content: chunks[i], embedding });
    }
  ...
  async search(query: string, k: number, _user: JwtUser): Promise<{ results: Citation[]; provider: string }> {
    ...
    const scored: Citation[] = chunks.map((c: any) => {
      ...
      return { doc_id: Number(c.docId), title: d?.title ?? '', source: d?.source ?? null, ord: c.ord, content: c.content, score: Math.round(cosine(qv, c.embedding as number[]) * 1000) / 1000 };
    });
    scored.sort((a, b) => b.score - a.score);
    return { results: scored.slice(0, Math.max(1, k)), provider: this.embedder.provider };
  }
