...
import { CpqBundlesService } from './cpq-bundles.service';
...
export class CpqService {
  ...
  constructor(
    ...
    // Ctor-body plain class (service-size headroom round) — bundles + guided-selling recommendations.
    this.bundles = new CpqBundlesService(db, (id) => this.assertConfig(id), (id) => this.assertQuote(id), (q) => this.fmtQuote(q));
  }
  private readonly bundles: CpqBundlesService;
  ...
  // ── CRM-14 (CRM-12): bundles + recommendations — extracted to CpqBundlesService (service-size
  // headroom round; ctor-body plain class). Facade keeps thin delegators; quote/config asserts stay here.
  async createBundle(dto: { code: string; name: string; description?: string; items: { config_id: number; qty?: number; unit_cost?: number }[] }, user: JwtUser) { return this.bundles.createBundle(dto, user); }
  async listBundles(user: JwtUser) { return this.bundles.listBundles(user); }
  async getBundle(code: string, user: JwtUser) { return this.bundles.getBundle(code, user); }
  async addBundleLine(quoteId: number, dto: { bundle_code: string; qty?: number; discount_pct?: number }, user: JwtUser) { return this.bundles.addBundleLine(quoteId, dto, user); }
  async recommendations(configCode: string, user: JwtUser) { return this.bundles.recommendations(configCode, user); }
