...
import { CrmAudienceService } from './crm-audience.service';
...
export class CrmService {
  constructor(
    ...
    // Ctor-body plain class (service-size headroom round) — the PDPA-05 audience/CDP egress surface.
    this.audience = new CrmAudienceService(db);
  }
  private readonly audience: CrmAudienceService;
  ...
  // ── PDPA-05 audience/CDP egress — extracted to CrmAudienceService (service-size headroom round;
  // ctor-body plain class). Thin delegators keep the controller + crm-bi-reports contracts unchanged.
  async exportForCustomerMatch(user: JwtUser, opts: { tenantId?: number | null; limit?: number; offset?: number } = {}) { return this.audience.exportForCustomerMatch(user, opts); }
  async upsertAudienceManifest(tenantId: number, rows: { member_id: number; hashed_email?: string; hashed_phone?: string; hashed_phone_plus?: string }[]) { return this.audience.upsertAudienceManifest(tenantId, rows); }
  async audienceRemovalCandidates(tenantId: number, limit = 5000) { return this.audience.audienceRemovalCandidates(tenantId, limit); }
  async markAudienceRemoved(tenantId: number, memberIds: number[]) { return this.audience.markAudienceRemoved(tenantId, memberIds); }
  async audienceExportRegister(user: JwtUser, limit = 50) { return this.audience.audienceExportRegister(user, limit); }
  async exportForCdp(user: JwtUser, opts: { tenantId?: number | null; limit?: number; offset?: number }) { return this.audience.exportForCdp(user, opts); }
