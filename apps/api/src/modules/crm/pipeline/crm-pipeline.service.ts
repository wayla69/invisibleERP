...
import { CrmLeadCaptureService } from './crm-lead-capture.service';
...
export class CrmPipelineService {
  ...
  private readonly leadCapture: CrmLeadCaptureService;
  ...
  constructor(
    ...
    // Ctor-body plain class (service-size headroom round) — public web-to-lead + bulk lead import.
    this.leadCapture = new CrmLeadCaptureService(db, docNo,
      (no, u) => this.scoreLead(no, u), (e, pl, u) => this.emitEvent(e, pl, u));
  ...
  // ── Lead capture (CRM-2) — extracted to CrmLeadCaptureService (service-size headroom round;
  // ctor-body plain class). Thin delegators keep the controller + public route contract unchanged.
  static readonly LEAD_IMPORT_HEADERS = CrmLeadCaptureService.LEAD_IMPORT_HEADERS;
  async webToLead(dto: { name: string; company?: string; email?: string; phone?: string; message?: string; source?: string; tenant_code?: string }) { return this.leadCapture.webToLead(dto); }
  async importLeads(input: { format?: 'rows' | 'csv' | 'xlsx'; csv?: string; xlsx?: string; rows?: Record<string, any>[]; dry_run?: boolean }, user: JwtUser) { return this.leadCapture.importLeads(input, user); }
