...
import { resolveEtaxProvider } from './etax-providers';
...
//     generic, configurable SP endpoint (see etax-providers.ts) — drop-in for INET/Frank/Leceipt once
//     a real contract + credentials exist; until then no vendor-specific shape is assumed.
@Injectable()
export class EtaxService {
  ...
  private async submitToProvider(provider: string, docNo: string, doc: { xml: string; signed: boolean }): Promise<{ status: string; providerRef: string; rd: any }> {
    const { provider: p } = resolveEtaxProvider(provider);
    return p.submit(docNo, doc);
  }
