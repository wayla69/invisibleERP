...
export class WmsService {
  ...
  async putaway(dto: { gr_no?: string; bin_code: string; item_id: string; lot_no?: string; qty: number; uom?: string; expiry_date?: string }, user: JwtUser) {
    ...
    await db.insert(stockMovements).values({ tenantId: user.tenantId ?? null, moveDate: new Date(), docNo: dto.gr_no ?? null, moveType: 'Transfer', itemId: dto.item_id, uom: dto.uom ?? null, qty: String(qty), fromLocation: 'Receiving', toLocation: `BIN:${dto.bin_code}`, refDoc: dto.gr_no ?? null, createdBy: user.username });
  ...
  async pick(pickNo: string, dto: { lines: { pick_line_id: number; picked_qty: number; bin_code?: string }[] }, user: JwtUser) {
    ...
    return await this.db.transaction(async (tx: any) => {
      ...
      for (const dl of dto.lines) {
        ...
        await tx.insert(stockMovements).values({ tenantId: user.tenantId ?? null, moveDate: new Date(), docNo: pickNo, moveType: 'Issue', itemId: line.itemId, uom: line.uom ?? null, qty: String(-want), fromLocation: `BIN:${bin.binCode ?? ''}`, toLocation: 'Shipping', refDoc: p.sourceRef, createdBy: user.username });
