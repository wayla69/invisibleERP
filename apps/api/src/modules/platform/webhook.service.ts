...
export class WebhookService {
  ...
  async register(dto: RegisterWebhookDto, user: JwtUser) {
    ...
    const [row] = await db.insert(webhooks).values({
      tenantId, url: dto.url, events, secret, active: true,
    }).returning({ id: webhooks.id });
    return { id: Number(row.id), url: dto.url, events: dto.events ?? [], secret };
  }
  ...
  async deliver(event: string, payload: unknown, tenantId: number | null) {
    ...
    for (const h of hooks) {
      ...
      const signature = 'sha256=' + createHmac('sha256', h.secret).update(body).digest('hex');
      try {
        const res = await fetch(h.url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-IERP-Signature': signature, 'X-IERP-Event': event },
          body,
        });
        const ok = res.ok;
