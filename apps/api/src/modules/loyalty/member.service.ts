...
export class MemberService {
  constructor(
    ...
    // Ctor-body plain class (god-service ratchet pattern) — the points lifecycle (expiry/sweep/look-ahead).
    this.lifecycle = new MemberLifecycleService(db, {
      accrueLiability: (a) => this.ledger.accrueLiability(a),
      recomputeTiersForTenant: (t, by) => this.recomputeTiersForTenant(t, by),
      expireLapsedVip: this.membership ? (t, by) => this.membership!.expireLapsed(t, by) : undefined,
      deliverWebhook: this.webhooks ? (e, pl, t) => this.webhooks!.deliver(e, pl, t) : undefined,
      runAutomation: this.automation ? (e, pl, u) => this.automation!.runEvent(e, pl, u) : undefined,
    });
  }
   ...
   * row lock the callers already hold; migration 0315's partial unique index is the hard backstop.
   */
  ...
  // Points lifecycle (expiry/breakage + maintenance sweep + W1 look-ahead) — extracted to
  // MemberLifecycleService (god-service ratchet round); GL accrual / VIP lapse / tier recompute /
  // webhook + automation fan-outs feed it as ports.
  async expirePoints(user: JwtUser, explicitTenantId?: number | null) { return this.lifecycle.expirePoints(user, explicitTenantId); }
  async sweepMaintenance(user: JwtUser, explicitTenantId?: number | null) { return this.lifecycle.sweepMaintenance(user, explicitTenantId); }
  async expiringForMember(memberId: number) { return this.lifecycle.expiringForMember(memberId); }
  async notifyExpiring(tenantId: number, user: JwtUser, lookAheadDays = 30) { return this.lifecycle.notifyExpiring(tenantId, user, lookAheadDays); }
