...
export const miAnalyticsSnapshots = pgTable('mi_analytics_snapshots', {
  ...
  // Model Governance (docs/60 Phase 4, migration 0468). Back-compat: status DEFAULTS to 'Approved' so a
...
// Budget Optimizer plans (migration 0465, docs/60 Phase 1). A prescriptive MMM allocation the planner
...
// Tenant-scoped: 0465 applies the canonical 0232-form org RLS loop + a leading (tenant_id, …) index.
...
// Closed-loop measurement (docs/60 Phase 3, migration 0467). Activating a pushed mi_segment splits the
