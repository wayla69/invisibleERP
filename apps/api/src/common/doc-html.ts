...
export function wrapA4(body: string, title: string): string {
...
export interface DocParty { name: string; address?: string | null; tax_id?: string | null; branch_label?: string | null; phone?: string | null; email?: string | null }

export function sellerHeaderHtml(p: DocParty): string {
  return `<div>
    <div class="t1">${esc(p.name)}</div>
    ${p.address ? `<div>${esc(p.address)}</div>` : ''}
    ${p.tax_id ? `<div>เลขประจำตัวผู้เสียภาษีอากร ${esc(formatTaxId(p.tax_id))} &nbsp; (${esc(p.branch_label ?? 'สำนักงานใหญ่')})</div>` : ''}
