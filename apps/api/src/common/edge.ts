...
type Bucket = 'otp' | 'auth' | 'lead' | 'signup' | 'api';
const bucketOf = (p: string): Bucket =>
  OTP_PATHS.has(p) ? 'otp' : AUTH_PATHS.has(p) ? 'auth' : SIGNUP_PATHS.has(p) ? 'signup' : PUBLIC_WRITE_PATHS.has(p) ? 'lead' : 'api';
...
export async function registerEdge(app: NestFastifyApplication): Promise<void> {
  ...
  await fastify.register(rateLimit, {
    ...
    max: (req: { url?: string }) => {
      ...
      return b === 'otp' ? OTP_MAX : b === 'auth' ? AUTH_MAX : b === 'signup' ? SIGNUP_MAX : b === 'lead' ? LEAD_MAX : GLOBAL_MAX;
    },
