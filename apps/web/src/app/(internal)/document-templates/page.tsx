...
export default function DocumentTemplatesPage() {
  ...
  const loadTemplate = (tpl: Tmpl) => { setSelectedId(tpl.id); setName(tpl.name); setCfg(hydrate(tpl.config)); setMsg(''); };
        ...
        <div className="grid gap-6">
          ...
          <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="grid gap-2"><Label htmlFor="show_logo">{t('st.dt.logo')}</Label><YesNo id="show_logo" value={cfg.header.show_logo} onChange={(v) => setCfg((c) => ({ ...c, header: { ...c.header, show_logo: v } }))} /></div>
              <div className="grid gap-2"><Label htmlFor="hdr">{t('st.dt.header_note')}</Label><Input id="hdr" value={cfg.header.header_note} onChange={(e) => setCfg((c) => ({ ...c, header: { ...c.header, header_note: e.target.value } }))} placeholder={t('st.dt.header_note_ph')} /></div>

              <div className="grid gap-2"><Label htmlFor="b_branch">{t('st.dt.branch')}</Label><YesNo id="b_branch" value={cfg.body.show_branch} onChange={(v) => setCfg((c) => ({ ...c, body: { ...c.body, show_branch: v } }))} /></div>
              <div className="grid gap-2"><Label htmlFor="b_addr">{t('st.dt.address')}</Label><YesNo id="b_addr" value={cfg.body.show_address} onChange={(v) => setCfg((c) => ({ ...c, body: { ...c.body, show_address: v } }))} /></div>
              <div className="grid gap-2"><Label htmlFor="b_tax">{t('st.dt.tax_id')}</Label><YesNo id="b_tax" value={cfg.body.show_tax_id} onChange={(v) => setCfg((c) => ({ ...c, body: { ...c.body, show_tax_id: v } }))} /></div>
              <div className="grid gap-2">
              ...
              <div className="grid gap-2"><Label htmlFor="fscale">{t('st.dt.font_scale')}</Label><Input id="fscale" type="number" step="0.05" min="0.8" max="1.4" value={cfg.body.font_scale} onChange={(e) => setCfg((c) => ({ ...c, body: { ...c.body, font_scale: Number(e.target.value) || 1 } }))} /></div>
              <div className="grid gap-2"><Label htmlFor="paper">{t('st.dt.paper_width')}</Label><Input id="paper" type="number" step="1" min="58" max="112" value={cfg.paper.width_mm} onChange={(e) => setCfg((c) => ({ ...c, paper: { width_mm: Number(e.target.value) || 80 } }))} /></div>

              <div className="grid gap-2 sm:col-span-2"><Label htmlFor="thanks">{t('st.dt.thanks')}</Label><Input id="thanks" value={cfg.footer.thanks_text} onChange={(e) => setCfg((c) => ({ ...c, footer: { ...c.footer, thanks_text: e.target.value } }))} placeholder={t('st.dt.thanks_ph')} /></div>
              <div className="grid gap-2 sm:col-span-2">
