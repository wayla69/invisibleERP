...
export default function EInvoicePage() {
  ...
  return (
    <div>
      <PageHeader title="ใบกำกับภาษีอิเล็กทรอนิกส์ (e-Invoicing)" description="ส่งใบกำกับไปยังหน่วยงานภาษีผ่านผู้ให้บริการแบบเสียบปลั๊ก (ค่าเริ่มต้นเป็น stub สำหรับทดสอบ) — บันทึกผลแบบ idempotent ไม่กระทบบัญชี" />
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><FileCheck className="size-4 text-primary" /> ส่งเอกสาร</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <StateView q={provs}>
              <div className="grid gap-1"><Label>ผู้ให้บริการ</Label>
              ...
              <div className="grid gap-1"><Label>เลขที่เอกสาร</Label><Input value={docRef} onChange={(e) => setDocRef(e.target.value)} /></div>
              <div className="grid gap-1"><Label>ยอดรวม</Label><Input type="number" value={total} onChange={(e) => setTotal(e.target.value)} /></div>
              <Button disabled={submit.isPending} onClick={() => submit.mutate()}>{submit.isPending ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />} ส่ง</Button>
              {msg && <p className="text-sm text-muted-foreground">{msg}</p>}
            </StateView>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base">ประวัติการส่ง</CardTitle></CardHeader>
          <CardContent className="overflow-x-auto">
            <StateView q={subs}>
              {(subs.data?.submissions ?? []).length === 0 ? <p className="text-sm text-muted-foreground">ยังไม่มี</p> : (
                <table className="w-full text-sm"><thead><tr className="border-b text-left text-muted-foreground"><th className="px-2 py-1 font-medium">เลขที่</th><th className="px-2 py-1 font-medium">Ref</th><th className="px-2 py-1 font-medium">สถานะ</th></tr></thead>
