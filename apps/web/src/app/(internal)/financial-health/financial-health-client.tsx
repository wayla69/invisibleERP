...
// Client island for the working-capital health dashboard. The parent RSC (page.tsx) server-prefetches
// the snapshot (cookie-forwarded) and passes it in as a prop; this component only renders + localizes it,
// so the read-only page keeps its server prefetch (matches projects/pipeline).
...
export function FinancialHealthClient({ data }: { data: any }) {
  const { t } = useLang();
