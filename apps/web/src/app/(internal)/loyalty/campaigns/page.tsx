...
interface Campaign { id: number; campaign_code: string; name: string; channel: string; audience: string; segment: string | null; tier: string | null; status: string; targeted: number; sent_count: number; skipped_count: number; failed_count: number; schedule_at: string | null }
          ...
          {list.data && (
            <DataTable
              ...
              columns={[
                ...
                { key: 'audience', label: 'กลุ่ม', render: (c) => <Badge variant="info">{c.audience}{c.segment ? `:${c.segment}` : c.tier ? `:${c.tier}` : ''}</Badge> },
