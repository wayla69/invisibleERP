...
function Leave() {
  ...
  const req = useMutation({
    ...
    onSuccess: () => { notifySuccess('ส่งใบลา'); setF({ ...f, days: '' }); refresh(); }, onError: (e: any) => notifyError(e.message),
  });
  ...
  return (
    <div className="grid gap-5">
      <Card className="gap-3 p-5">
        ...
        <div className="flex items-center gap-3"><Button onClick={() => req.mutate()} disabled={!f.emp_code || !f.days || req.isPending}><Plane className="size-4" /> ส่งใบลา</Button></div>
      </Card>
      ...
      ]} emptyState={{ icon: Plane, title: 'ยังไม่มีใบลา', description: 'ส่งใบลาจากแบบฟอร์มด้านบน คำขอที่รออนุมัติจะแสดงที่นี่' }} />}</StateView>
    </div>
  );
}
