...
export default function AutomationPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { setMsg(t('st.auto.rule_created')); setF({ ...f, name: '', condField: '', condValue: '', message: '' }); qc.invalidateQueries({ queryKey: ['automation-rules'] }); },
    onError: (e: any) => setMsg(`❌ ${e.message}`),
  });
  const del = useMutation({
    mutationFn: (id: number) => api(`/api/automation/rules/${id}`, { method: 'DELETE' }),
    onSuccess: () => { setMsg(t('st.auto.rule_deleted')); qc.invalidateQueries({ queryKey: ['automation-rules'] }); },
    onError: (e: any) => setMsg(`❌ ${e.message}`),
  });
  const runTest = useMutation({
    ...
    onSuccess: (r: any) => { setMsg(t('st.auto.test_done', { matched: r.matched, executed: r.executed })); qc.invalidateQueries({ queryKey: ['automation-execs'] }); },
    onError: (e: any) => setMsg(`❌ ${e.message}`),
  });

  return (
    <div>
      <PageHeader title={t('st.auto.title')} description={t('st.auto.subtitle')} />
      {msg && <div className="mb-3"><Msg ok={msg.startsWith('✅') || msg.startsWith('🗑️')}>{msg}</Msg></div>}

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Plus className="size-4 text-primary" /> {t('st.auto.new_rule')}</CardTitle></CardHeader>
      ...
      <Card className="mt-6">
        <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Workflow className="size-4 text-primary" /> {t('st.auto.configured_rules')}</CardTitle></CardHeader>
        <CardContent>
          <StateView q={rules}>
            {(rules.data?.rules ?? []).length === 0 ? <p className="text-sm text-muted-foreground">{t('st.auto.no_rules')}</p> : (
              <div className="grid gap-2">
      ...
      <Card className="mt-6">
        <CardHeader><CardTitle className="text-base">{t('st.auto.exec_history')}</CardTitle></CardHeader>
        <CardContent>
          <StateView q={execs}>
            <div className="grid gap-1 text-sm">
              ...
              {(execs.data?.executions ?? []).length === 0 && <p className="text-muted-foreground">{t('st.auto.no_execs')}</p>}
            </div>
          </StateView>
        </CardContent>
      </Card>
