...
async function main() {
  ...
  // ── 14-16. per-ledger year-end close — NATURAL order (leading/TFRS first, which locks T1's periods;
  // the non-leading TAX close must still succeed via allowClosedPeriod). Proves order-independence.
  // Periods/close are per-tenant (0043): HQ admin targets the shop's books with &tenant_id=T1. ──
  const closeTfrs = await inj('POST', `/api/ledger/close-year?fiscal_year=2026&tenant_id=${t1}`, admin);
  const closeTax = await inj('POST', `/api/ledger/close-year?fiscal_year=2026&ledger=TAX&tenant_id=${t1}`, admin);
