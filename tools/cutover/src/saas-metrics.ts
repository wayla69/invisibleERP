...
async function main() {
  ...
  // ── PRICE GRANDFATHERING (0455, docs/53 Q7): a subscription's snapshotted price survives a plan-row
  ...
  ok('0455: getSubscription returns the EFFECTIVE price (snapshot 1,900), list price beside it, flagged',
  ...
  ok('0455: checkout of the CURRENT plan charges the grandfathered ฿1,900, not list ฿2,900', near(gfCk.amount, 1900), `amount=${gfCk.amount}`);
  const otherCk = await g(() => billing.createCheckoutSession(t3, 'business', 'monthly'));
  ok('0455: checkout of a DIFFERENT plan prices at current list (฿4,900 — no snapshot carry-over)', near(otherCk.amount, 4900), `amount=${otherCk.amount}`);
  ...
  ok('0455: MRR uses each sub\'s effective price (9,900 + 9,900 + 1,900 = 21,700)', near(m2.revenue?.mrr, 21700), `mrr=${m2.revenue?.mrr}`);
  ...
  ok('0455: a plan change RE-SNAPSHOTS at the new plan\'s current price (฿4,900) — old lock ends',
    gfCh.plan === 'business' && near(Number(gfRow.grandfathered_price), 4900), JSON.stringify(gfRow));

  // ── PRODUCT-LINE SKUs (0456, docs/53 C1): POS line prices PER BRANCH — checkout multiplies by the
  ...
  ok('0456: POS Pro × 3 branches checkout charges 3 × ฿1,190 = ฿3,570 and persists branches=3',
  ...
  ok('0456: POS Lite × 2 branches annual = 2 × ฿5,900 = ฿11,800 (per-branch × 10-month annual)',
    near(pbAnnual.amount, 11800), `amount=${pbAnnual.amount}`);
  ok('0456: a flat (per-company) plan rejects an explicit branch quantity → PLAN_NOT_PER_BRANCH',
