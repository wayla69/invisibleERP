...
async function main() {
  ...
  // ───────────────────── AR cash application (REV-20) — multi-invoice / on-account / CN-as-credit ─────────────────────
  ...
  ok('REV-20: suggest — exact single-invoice match wins (500 → INV-CA2)', sgExact.exact_match === true && sgExact.lines?.length === 1 && sgExact.lines?.[0]?.invoice_no === 'INV-CA2' && near(sgExact.lines?.[0]?.apply, 500), JSON.stringify(sgExact.lines));
  const sgFifo = (await inj('GET', `/api/finance/ar/cash-application/suggest?customer_no=CAPP&amount=900`, admin)).json;
  ok('REV-20: suggest — no exact match ⇒ oldest-due-first (600 on CA1, 300 partial on CA2)', sgFifo.exact_match === false && sgFifo.lines?.[0]?.invoice_no === 'INV-CA1' && near(sgFifo.lines?.[0]?.apply, 600) && sgFifo.lines?.[1]?.invoice_no === 'INV-CA2' && near(sgFifo.lines?.[1]?.apply, 300), JSON.stringify(sgFifo.lines));
  ...
  ok('REV-20: one receipt across two invoices; remainder parks on-account (applied 900 / on-account 100)',
  ...
  ok('REV-20: sub-ledger moved per line (CA1 Paid 600, CA2 Partial 300)', capInv1?.status === 'Paid' && near(capInv1?.paidAmount, 600) && capInv2?.status === 'Partial' && near(capInv2?.paidAmount, 300), `ca1=${capInv1?.status}/${capInv1?.paidAmount} ca2=${capInv2?.status}/${capInv2?.paidAmount}`);
  ok('REV-20: GL — Dr 1000 +1000, Cr 1100 −900, Cr 2220 −100 (on-account liability); receipt carries unapplied 100',
  ...
  ok('REV-20: aging reflects applications + surfaces the on-account credit (CA1 gone; on_account ≥ 100; net_total = total − on_account)',
  ...
  ok('REV-20: application beyond the invoice open balance → 400 OVER_APPLIED', capOver.status === 400 && capOver.json?.error?.code === 'OVER_APPLIED', `${capOver.status} ${capOver.json?.error?.code}`);
  ...
  ok('REV-20: Σ applications beyond the receipt → 400 APPLY_EXCEEDS_RECEIPT', capExceed.status === 400 && capExceed.json?.error?.code === 'APPLY_EXCEEDS_RECEIPT', `${capExceed.status} ${capExceed.json?.error?.code}`);
  ...
  ok('REV-20: another customer\'s invoice → 400 CUSTOMER_MISMATCH', capXcust.status === 400 && capXcust.json?.error?.code === 'CUSTOMER_MISMATCH', `${capXcust.status} ${capXcust.json?.error?.code}`);
  ...
  ok('REV-20: open-items worksheet feed lists the open invoices + the unapplied receipt', (capOI.invoices ?? []).some((r: any) => r.invoice_no === 'INV-CA2' && near(r.available, 200)) && (capOI.unapplied_receipts ?? []).some((r: any) => r.receipt_no === capRcpNo && near(r.available, 100)) && near(capOI.totals?.on_account, 100), JSON.stringify(capOI.totals));
  ...
  ok('REV-20: apply-on-account later moves the parked cash onto the invoice (2220 → 1100)',
  ...
  ok('REV-20: applying more than the remaining on-account cash → 400 INSUFFICIENT_UNAPPLIED', capInsuff.status === 400 && capInsuff.json?.error?.code === 'INSUFFICIENT_UNAPPLIED', `${capInsuff.status} ${capInsuff.json?.error?.code}`);
  ...
  ok('REV-20: a ≥100k application parks PendingApproval — invoice untouched, cash fully on-account (Cr 2220)',
  ...
  ok('REV-20: the parked batch surfaces in the GOV-01 pending-approvals monitor', (capGov.items ?? []).some((i: any) => i.type === 'ar_cash_application' && i.control === 'REV-20' && i.ref === capBig.json?.batch_no && near(i.amount, 150000)), `types=${JSON.stringify(capGov.by_type ?? {})}`);
  const capSelf = await inj('POST', `/api/finance/ar/cash-application/${capBig.json?.batch_no}/approve`, admin);
  ok('REV-20: poster self-approval blocked → 403 SOD_VIOLATION', capSelf.status === 403 && capSelf.json?.error?.code === 'SOD_VIOLATION', `${capSelf.status} ${capSelf.json?.error?.code}`);
  ...
  ok('REV-20: a different user approves → invoice settles + GL relief posts (Dr 2220 / Cr 1100 150000)',
  ...
  ok('REV-20: reversal without a reason rejected → 400 REASON_REQUIRED', capRevNoReason.status === 400 && capRevNoReason.json?.error?.code === 'REASON_REQUIRED', `${capRevNoReason.status} ${capRevNoReason.json?.error?.code}`);
  ...
  ok('REV-20: reversal reopens the invoice + returns the cash on-account (Dr 1100 / Cr 2220), audited with reason',
  ...
  ok('REV-20: the application register carries the audited reversal (reversed flag + reason + who)', (capAppReg.applications ?? []).some((a: any) => a.application_no === capApl1 && a.reversed === true && a.reversed_by === 'admin' && (a.reverse_reason ?? '').includes('โต้แย้ง')), `n=${capAppReg.count}`);
  const capRevAgain = await inj('POST', `/api/finance/ar/cash-application/${capApl1}/reverse`, admin, { reason: 'ซ้ำ' });
  ok('REV-20: a reversed application cannot be reversed again → 400 ALREADY_REVERSED', capRevAgain.status === 400 && capRevAgain.json?.error?.code === 'ALREADY_REVERSED', `${capRevAgain.status} ${capRevAgain.json?.error?.code}`);
  ...
  ok('REV-20: open-items surfaces the applicable AR-linked credit note (remaining 50)', (capOI2.credit_notes ?? []).some((c: any) => c.doc_no === 'CN-CAPP-1' && near(c.remaining, 50)), JSON.stringify(capOI2.credit_notes));
  ...
  ok('REV-20: credit note applies as a credit line — invoice open balance falls 50, NO new GL (the note posted its own at approval)',
  ...
  ok('REV-20: exhausted credit note → 400 CN_OVER_APPLIED', capCnOver.status === 400 && capCnOver.json?.error?.code === 'CN_OVER_APPLIED', `${capCnOver.status} ${capCnOver.json?.error?.code}`);
  const capStmt = (await inj('GET', `/api/finance/ar/statement?tenant_id=${capTid}`, admin)).json;
  ok('REV-20: customer statement carries the applied credit note as a credit line (type credit_note, 50)', (capStmt.lines ?? []).some((l: any) => l.type === 'credit_note' && near(l.payment, 50) && l.ref === 'CN-CAPP-1'), `n=${capStmt.lines?.length}`);
  const capWl = (await inj('GET', '/api/finance/ar/collections', admin)).json;
  ok('REV-20: collections worklist surfaces the customer\'s on-account cash (apply before dunning)', (capWl.rows ?? []).some((r: any) => r.invoice_no === 'INV-CA1' && near(r.on_account, 600)) && Number(capWl.on_account_total) >= 600, `oa_total=${capWl.on_account_total}`);

  console.log('\n── ERP basics — Cash Flows + Collections/Dunning + ESS-AP + EAM + credit/depth/forecast + recurring + statements/petty-cash/prepaid/lease/revaluation + inventory sub-ledger + FIFO/FEFO + industry CoA + GL-12 posting-rules engine + GL-13 multi-dim postings + GL-14 sub-ledger tie-out + GL-15/GL-16 hard period close + C1 multi-currency (JPY 0dp) + C2 pluggable tax (SG/MY/EU) + e-invoicing (MyInvois/Peppol) + REV-20 AR cash application ──');
