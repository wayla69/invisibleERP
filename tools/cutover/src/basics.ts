...
async function main() {
  ...
  // ───────── CRM sales pipeline: leads → opportunities (stage machine) → activities (REV-16) ─────────
  ...
  ok('REV-16: create a lead (LEAD-…)', lead.status === 201 && /^LEAD-/.test(lead.json?.lead_no ?? ''), JSON.stringify(lead.json));
  ...
  ok('REV-16: convert lead → customer-of-record (CUS-) + opportunity (OPP-)', conv.status === 201 && conv.json?.status === 'converted' && /^CUS-/.test(conv.json?.customer_no ?? '') && /^OPP-/.test(conv.json?.opp_no ?? ''), JSON.stringify(conv.json));
  const oppNo = conv.json?.opp_no;
  const convAgain = await inj('POST', `/api/crm/pipeline/leads/${leadNo}/convert`, admin, {});
  ok('REV-16: a converted lead cannot be re-converted (LEAD_CONVERTED)', convAgain.status === 400 && convAgain.json?.error?.code === 'LEAD_CONVERTED', `st=${convAgain.status} code=${convAgain.json?.error?.code}`);
  ...
  ok('REV-16: advance opportunity through the stage machine → won (probability 100)', won.status === 200 && won.json?.stage === 'won' && won.json?.probability === 100, JSON.stringify(won.json));
  const reWon = await inj('PATCH', `/api/crm/pipeline/opportunities/${oppNo}/stage`, admin, { stage: 'negotiation' });
  ok('REV-16: a closed (won) opportunity is terminal (OPP_CLOSED)', reWon.status === 400 && reWon.json?.error?.code === 'OPP_CLOSED', `st=${reWon.status} code=${reWon.json?.error?.code}`);
  ...
  ok('REV-16: marking lost requires a reason (LOST_REASON_REQUIRED)', lostNoReason.status === 400 && lostNoReason.json?.error?.code === 'LOST_REASON_REQUIRED', `st=${lostNoReason.status} code=${lostNoReason.json?.error?.code}`);
  const pipe = (await inj('GET', '/api/crm/pipeline/summary', admin)).json;
  ok('REV-16: weighted pipeline forecast + won total computed', near(pipe.won_amount, 100000) && pipe.weighted_forecast >= 20000 && (pipe.by_stage?.won?.count ?? 0) >= 1, JSON.stringify({ won: pipe.won_amount, wf: pipe.weighted_forecast }));
  ...
  ok('REV-16: log + list an activity against the opportunity', (acts.activities ?? []).some((a: any) => a.type === 'call' && a.subject === 'Kickoff call'), `n=${acts.count}`);
