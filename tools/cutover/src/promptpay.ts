...
async function main() {
  ...
  // ── 3. set a valid mobile id ──
  const set = await inj('PATCH', '/api/tenant/profile', admin, { promptpay_id: '0812345678' });
  ok('Set PromptPay id on tenant profile', set.status < 300 && set.json.promptpay_id === '0812345678', JSON.stringify({ s: set.status, pp: set.json.promptpay_id }));
  ...
  // ── 6. national-id (13-digit) target also formats ──
  await inj('PATCH', '/api/tenant/profile', admin, { promptpay_id: '1234567890123' });
